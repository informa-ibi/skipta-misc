...
*/
	...
	{
		echo "\n\t". '<div class="entry">';
		echo "\n\t". category_description(); // Check if there is content
		echo "\n\t". '</div><!-- /entry -->';   		
	}
	...
	echo "\n\t". '<div id="blog_container" class="acoda-ajax-container row '. ( $blog_search == 'search_filter' ? ' filter search' : '' ) . ( $masonry == 'masonry' ? ' '. esc_attr( $masonry ) : '' ) . ( $acoda_postlayout == 'grid' || $acoda_postlayout == 'masonrygrid' ? ' grid' : '' ) .'" data-source="'. esc_attr( $query_type ) .'" data-load-method="'. esc_attr( $paginate ) .'"  data-query="'. esc_attr( $query ) .'" data-ajaxurl="'. esc_url( admin_url( 'admin-ajax.php' ) ) .'" data-postlayout="'. ( $acoda_postlayout == 'grid' || $acoda_postlayout == 'masonrygrid' ? 'grid' : 'normal' ) .'" data-grid-columns="'. esc_attr( $acoda_gridcols ) .'">';
	...
	elseif( $blog_header !== true && $post_count == 0 ):
