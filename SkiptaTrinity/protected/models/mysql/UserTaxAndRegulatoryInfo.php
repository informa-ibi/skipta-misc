...
//    public $LicensedNumber;
    ...
    public function saveUserInfo($model) {
        try {
            ...
            $userObj->NPIStatus = $model->NPIStatus;
          ...
          //  $iv = mcrypt_create_iv(mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB), MCRYPT_RAND);
           // $userObj->FederalTaxIdOrSSN = mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $secret_key, $string, MCRYPT_MODE_CBC, $iv);
            ...
            $userObj->NPIState = $model->NPIState;
            if($userObj->save()){
...
//                $return = LicensedStatesAndNumbers::model->saveObject();
                ...
                if(!empty($model->NPIState) && sizeof($model->NPIState)>0){
                    NPIStatesAndNumber::model()->saveData($pid,$model);
                }
                $returnValue = User::model()->updateUserById($model);
      ...
      //-----End-----------------      
            ...
            error_log("=^^^^^^^^^*******===Exception ====saveUserInfo=====".$exc->getMessage());
    ...
    public function getProfileDetails($userId,$scheduleId,$surveyId){
        try{
            ...
            error_log("=^^^^^^^^^*******===Exception ====getProfileDetails=====".$ex->getMessage());
        }
    }
...
//                        $user->PasswordResetToken = 'reset';
    ...
    public function getSurveyTakenUsersInfoData($surveyId,$scheduleId){
        try{
            ...
            error_log("==getSurveyTakenUsersInfoData==query===$query");
            $results = Yii::app()->db->createCommand($query)->queryAll(); 
            if(count($results)>0){
                 $returnValue= $results;
             }
            ...
            error_log("=====getSurveyTakenUsersInfoData======".$ex->getMessage());
             Yii::log("getSurveyTakenUsersInfoData=" . $ex->getMessage(), "error", "application");
        }
    }
