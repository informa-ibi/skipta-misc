...
class CuratedNewsCollection extends EMongoDocument{
  ...
  public function saveComment($postId,$comments){
    ...
    public function getPostObjectFollowers($postId){
        ...
        try {
            ...
            Yii::log("==getPostObjectFollowers In curatedNews collection==".$exc->getMessage(), 'error', 'application');
        }
        return $returnValue;
  ...
  public function getInvitedUsersForPost($postId) {
        ...
        try {
            ...
            Yii::log("==getInvitedUsersForPost In Curated collection==" . $exc->getMessage(), 'error', 'application');
            return $returnValue;
        }
        return $returnValue;
    }
...
//                'Title' => array('eq' => new MongoRegex('/' . $searchText . '.*/i')),
        ...
        foreach ($posts as $post) {
            ...
            if($postLength>240){
                 $post->Description =  CommonUtility::truncateHtml($post->Description, 240,'Read more',true,true,' <i class="fa moreicon">'.Yii::t('translation','Readmore').'</i>'); 
