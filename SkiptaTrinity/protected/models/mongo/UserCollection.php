...
class UserCollection extends EMongoDocument {
   ...
   public function getTinyUserCollection($userId){
       try {
          $returnValue='failure';
           ...
           error_log("_____________________________in coll________________".$exc->getMessage());
 ...
 public function updateUserCollection($userCollection,$updateCountry){
      $returnValue='failure';
      try {
            $mongoCriteria = new EMongoCriteria;
            $mongoModifier = new EMongoModifier;
            ...
            if($updateCountry){
               $mongoModifier->addModifier('State', 'set', $userCollection->State);            
...
public function getAllNotLoggedInUsersFromPastFourdays(){
    ...
    public function updateProfileDetails($UserprofileModel) {
        ...
        try {
            ...
            $mongoModifier->addModifier('AboutMe', 'set', $UserprofileModel['AboutMe']);
             $mongoModifier->addModifier('State', 'set', $UserprofileModel['State']);
 ...
 public function getTinyUserCollectionTest($userId){
       try {
          $returnValue='failure';
          
            $mongoCriteria = new EMongoCriteria;            
            $mongoCriteria->addCond('UserId', '==', (int)19);
            $tinyUserObj = UserCollection::model()->findAll($mongoCriteria);
            return  $tinyUserObj;
       }  
       catch (Exception $ex) {
  ...
  public function updateUserTinyCollection($user){
      ...
      try {
            ...
            $mongoModifier->addModifier('CountryId', 'set', (int)$user->Country);
