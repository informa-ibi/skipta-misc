...
//            }
            ...
            Yii::log("______in get findGroupBySubgroupId_______" . $exc->getMessage(), 'error', 'application');
            return $returnValue;
        }
    }
...
//            }
    ...
    public function updateGroupUniqueName($id,$gname){
        try{
            ...
            error_log("=#########Exception Occurred while updating the unique name###########".$ex->getMessage());
        }
    }
...
//               $mongoModifier->addModifier('IsHybrid', 'set', (int)0);
    ...
    public function userUnfollowAllGroups($UserId) {
        try {
            ...
            Yii::log("---------inside exception ---userUnfollowAllGroups--------------" . $ex->getMessage(), 'error', 'application');
        }
   ...
   public function getGroupPostIds($groupId){
       ...
       try {
           ...
           error_log("------in get grouppostids----".$exc->getMessage());
       }
      }
   ...
   public function checkGroupStatus($groupId){
       ...
       try {         
            ...
            return $returnValue;
             ...
             $returnValue = 'failure';
           error_log("check group Status".$exc->getMessage());
       }
      }
/**
    * @author vamsi krishna
