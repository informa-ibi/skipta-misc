...
//             $criteria->addCond('Location', '!=', "undefined,undefined");
             $criteria->addCond('Country', '!=', "undefined");
//             if (is_array($NetworkId) && sizeof($NetworkId) > 0) {
//                $criteria->addCond('NetworkId','in',$NetworkId);
//            }
            
            $criteria->Date = array('$gte' => $startDate, '$lte' => $endDate);
            $Locations = TrackBrowseDetailsCollection::model()->findAll($criteria);
           
            if(sizeof($Locations)>0){
                foreach ($Locations as $key => $value) {
                    $latlang=  explode(',',$value['Location']);
                    error_log($latlang[1]."------------------".$latlang[0]);
                    $finalArray['lattitude'][$key] = $latlang[0];
                     $finalArray['longitude'][$key] = $latlang[1];
                }
                if(sizeof($finalArray)>0){
                    $returnvalue=$finalArray;
                }
            }
            error_log("in collectionnnnnnn-----------4-----------------".print_r($returnvalue,true));
            return $returnvalue;
        } catch (Exception $exc) {
            Yii::log($exc->getMessage(), 'error', 'application');
        }
