...
class GameCollection extends EMongoDocument {
 ...
 public function getGameDetailsByType($type,$value){
     try {
         ...
         Yii::log("in getGameDetailsByType".$exc->getMessage(),'error','application');
     }
  }
   /**
     * @author Vamsi Krishna
   ...
   public function getGameDetailsObject($type,$value){
     try {
         ...
         Yii::log("getGameDetailsObject____".$exc->getMessage(),'error','application');
     }
  }
   /**
     * @author Vamsi Krishna
  ...
  public function getGameCommentsByPostId($postId){
    ...
    public function UpdateGame($gameId,$Gameobj) {
        try {
            ...
            Yii::log("UpdateGame" . $exc->getMessage(), 'error', 'application');
        }
    }
     ...
     public function UpdateGameQuestions($gameId,$Gameobj) {
        try {
            ...
            Yii::log("UpdateGameQuestions" . $exc->getMessage(), 'error', 'application');
        }
    }
     public function UpdateGameDetails($Gameobj) {
        try {
          ...
          $modifier->addModifier('QuestionsCount', 'set', count($Gameobj->Questions));
            ...
            Yii::log("UpdateGameDetails" . $exc->getMessage(), 'error', 'application');
        }
    }
    ...
    public function getPostCommentsByPostId($postId) {
     ...
     public function updateGameForIsCurrentSchedule($type,$value,$gameId) {
        try {
            ...
            $mongoModifier = new EMongoModifier;
            if($type == "IsCurrentSchedule")
            $mongoModifier->addModifier('IsCurrentSchedule', 'set', $value);
     ...
     public function UpdateGameFields($gameId,$Gameobj,$Fieldvalue) {
        try {
            ...
            Yii::log("UpdateGameFields" . $exc->getMessage(), 'error', 'application');
        }
    }
 ...
 public function getGameName($id){
     try {
         ...
         Yii::log("in getGameName".$exc->getMessage(),'error','application');
     }
  }
 ...
 public function isCurrentScheduleGame($gameId){
     try {
         ...
         Yii::log("in isCurrentScheduleGame".$exc->getMessage(),'error','application');
     }
  }  
 public function suspendORReleaseGame($gameId,$suspendId) {
        try {
            ...
            Yii::log("in suspendORReleaseGame" . $exc->getMessage(), 'error', 'application');
        }
    }
...
//                    }
                ...
                if (sizeof($postObj) > 0) {
                    ...
                    foreach ($postObj as $post) {
                        ...
                        if ($postLength > 240) {
                            ...
                            $post->GameDescription = $post->GameDescription . ' <i class="fa  moreicon">'.Yii::t('translation','Readmore').'</i>';
                        }
    ...
    public function getGamesForSearch($searchText, $offset, $pageLength) {
        try {
            ...
            foreach ($posts as $post) {
                ...
                if ($postLength > 240) {
                    $post->GameDescription = CommonUtility::truncateHtml($post->GameDescription, 240, 'Read more', true, true, ' <i class="fa  moreicon">'.Yii::t('translation','Readmore').'</i>');
    ...
    public function commentManagement($commentId, $postId, $actionType, $userId=""){
     ...
     try {
         ...
         Yii::log("in getRecentGameDetailsObject".$exc->getMessage(),'error','application');
     }
  }
