   ...
   public function indexes() {
    ...
    public function saveGoogleAnalyticsData($reportDate,$pageviews,$pagevisits,$AvgTimeOnSite,$DayOfHour) {
        try {
            ...
            Yii::log($exc->getMessage() . "In save google analytics data collection", 'error', 'application');
        }
    }
     public function CheckGoogleAnalyticsDataExistByDate() {
        try {
            ...
            $criteria->addCond('CreatedOn', '==', new MongoDate(strtotime(date('Y-m-d'))));
...
return $resArray;
      ...
      public function saveGroupWiseGoogleAnalyticsData($reportDate,$pageviews,$pagevisits,$AvgTimeOnSite,$DayOfHour,$groupId) {
        try {
            $returnValue = 'failure';
            $AnalyticsObj = new GoogleAnalyticsCollection();
            $AnalyticsObj->Pageviews = $pageviews;
            ...
            Yii::log($exc->getMessage() . "In save  group google analytics data collection", 'error', 'application');
        }
    }
       ...
       public function saveGoogleAnalyticsPastData($reportDate,$pageviews,$pagevisits,$AvgTimeOnSite,$DayOfHour) {
        try {
          ...
          $AnalyticsObj->CreatedOn = new MongoDate(strtotime($reportDate));
            ...
            Yii::log($exc->getMessage() . "In save google analytics past data collection", 'error', 'application');
        }
    }  
