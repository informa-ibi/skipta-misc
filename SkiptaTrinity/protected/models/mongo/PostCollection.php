 ...
 /**
   ...
   try {
        ...
        foreach ($posts as $post) {
            ...
            error_log("anaonysss-----".$post->IsAnonymous);
    ...
    public function getPostsForHashtag($hashtagId, $offset, $pageLength) {
        ...
        foreach ($posts as $post) {
            ...
            if($postLength>240){
                 ...
                 $post->Description = $post->Description.' <i class="fa  moreicon">'.Yii::t('translation','Readmore').'</i>';
            }
     ...
     */
     public function getAllStreamTypePostsByUserId($userId){
        try {       
         ...
         try{
             error_log("Postcollection----".$id."---act--".$actionType);
             $criteria = new EMongoCriteria;
