   ...
   'PreviousScheduleResumePlayers'=>'PreviousScheduleResumePlayers',
    ...
    public function saveUserStream($obj) {
        try {
            ...
            error_log( '__________segmentid____________'.$obj->SegmentId);
...
// End Game resource update in UserStream collection
  ...
  /**
    ...
    */
    public function getUserAttendingEventsActivity($userId,$CurrentLoginDate, $segmentId=0)
    {       
        try
        {   
  ...
  public function updateStreamForGroupInactiveAndActive($obj) {
        try {
            ...
            error_log("---------------in stream coleltopn------------------------------".$exc->getMessage());
            Yii::log("------" . $exc->getMessage(), 'error', 'application');
        }
    }
...
public function  getNewlyRegisteredUsersCompletedSurveysByAdvertisementId($advertisementId) {
   ...
   * @return type
   */
  public function getPostType($postId,$categoryType){
      try{
        
          $postType = "failure";
           $mongoCriteria = new EMongoCriteria;
           if($categoryType == 9){
                $mongoCriteria->setSelect(array('GameName'=>true));
                $mongoCriteria->addCond("CurrentGameScheduleId","==",new MongoId($postId));  
              
           }else{
               $mongoCriteria->setSelect(array('PostType'=>true));
               $mongoCriteria->addCond("PostId","==",new MongoId($postId));  
           }
         
          $obj = UserStreamCollection::model()->find($mongoCriteria);
          //error_log("post ype==".$obj->PostType);
          if(is_object($obj)){
              if($categoryType == 9){
                 $postType =  $obj->GameName;  
              }else{
...
Yii::log("UserStreamCollection:getPostType::".$ex->getMessage()."--".$ex->getTraceAsString(), 'error', 'application');
      }
  }
        }
