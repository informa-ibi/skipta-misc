                ...
                if ($data->CategoryType == 9) {
                    try {
                        ...
                        if ($UserId == $data->OriginalUserId) {
                          ...
                          error_log("@@@@@@@@@GSR@@@@@@@@@@@@@@@@@@@@@@@@".$exc->getTraceAsString()); 
                    }
                }else {
                    ...
                    if ($data->ActionType == "Post") {
                        ...
                        if ($data->PostType == 2) {
                           // $data->Title = "";
                            ...
                            // $data->Title = "";
                            $data->StreamNote = "A new survey has been created"; 
                        }else{
                             $data->StreamNote = "A new post has been created";
                        }
                    }
                ...
                if (sizeof($comments) > 0) {
                    ...
                    for ($j = sizeof($comments); $j > 0; $j--) {
                        ...
                        if(!$IsBlockedWordExist && !$IsAbused){
                            ...
                            $comment["CommentFullText"] = CommonUtility::findUrlInStringAndMakeLink($comment["CommentFullText"]);
...
//            }
            ...
            error_log("__________exception_________________________________" .$ex->getLine(). $ex->getMessage());
   ...
   static function prepareDSNStreamObject($userId, $actionType, $postId, $categoryId, $networkName, $description, $networkLogo )
            ...
            foreach ($streamPostData as $key => $data) {
                ...
                if (($data->PostType < 7 || $data->PostType == 11 || $data->PostType == 12) && $data->RecentActivity != "UnFollow" ) {
                    ...
                    if($recentActivity=="Invite"){
                        ...
                        error_log($UserId."===============================".$data->InviteUsers);
                        if($data->Priority!=0){
                            $isInvited = 1;
                        }
                        $InviteUserName=$tinyOriginalUser['DisplayName'];
    ...
    static function prepareFeaturedPostData($userId, $abusedPostData, $categoryType) {
        try {
            foreach ($abusedPostData as $data) {
                error_log("&&&ana------".$data->IsAnonymous);
...
//                $custompostText=$postText;                
            ...
            error_log("===totalNotifications====$isNHistory==");
            return $totalArray;
        } catch (Exception $ex) {
            error_log($ex->getMessage());
        }
...
//
    ...
    public static function prepareFeaturedItemsForMobile($providerData){
        ...
        if (sizeof($providerData) != 0) {
            ...
            error_log("prepareFeaturedItemsForMobile---".$ex->getMessage());
        }
        return $returnValue;
    }
        ...
        try {
            ...
            $userAnswers = $scheduleObject->UserAnswers;
             ...
             foreach ($uncompletedAnswers as $uncompleteAnswer) {
                 //error_log("***************---".  print_r($uncompleteAnswer,1));
                 if(is_array($uncompleteAnswer) && sizeof($uncompleteAnswer)>0){
                     //error_log("iffffffffffffff merfe");
...
//                            }
                 ...
                 error_log($question["QuestionId"]."===total count-------------" . $totalCount."----".$userCount);
                ...
                if ($question['QuestionType'] == 3 || $question['QuestionType'] == 4) {
                    ...
                    $optionsArray = $question['OptionName'];
                      ...
                      //error_log("JustificationPlaceholders---------". sizeof($JustificationPlaceholders));
                    ...
                    //error_log("@@@@@@@@@@@@======$$$$$$$$$$===".print_r($optionsArray,1));
                    foreach ($optionsArray as $option) {
                        $emptyArray = array();
                   ...
                   foreach ($JustificationPlaceholders as $key=>$optionComment) {
                    ...
                    //error_log("---".print_r($optionsCommentsNewArray,1));
                    ...
                    foreach ($answersArray as $value) {
                        ...
                        if ($question['QuestionId'] == $value["QuestionId"]) {   
                            $selectedOptionArray = $value['Options'];
                             ...
                             //error_log("selctec commentarray--------".print_r($selectedOptionCommnetsArray,true));
...
//                                    }
                            ...
                            //error_log( $value["QuestionId"]."===option commetn array before-----------".print_r($optionsCommentsNewArray,true));
                    ...
                    if(isset($sop) && !empty($sop)){
                       ...
                       $optionsCommentsNewArray[$key] = $val+1;
                             ...
                             //error_log( $value["QuestionId"]."---option commetn array after-----------".print_r($optionsCommentsNewArray,true));
                            $question['OptionsNewArray'] = $optionsNewArray;
...
//                                      error_log($question['QuestionType']."==anws----------".$k."----".$v);
...
//                        }
                           ...
                           $optionsCommentsPercentageArray = array();
                             ...
                             //error_log("sum commetns------------".$sumComments);
                    ...
                    //error_log("percdnte-------".print_r($optionsCommentsPercentageArray,1));
                    $question['OptionsPercentageArray'] = $optionsPercentageArray;
...
//                    error_log("options percentage array---" . print_r($optionsPercentageArray, true));
                ...
                if ($question['QuestionType'] == 6) {
                    ...
                    //error_log("quesiotn0d----".$question['QuestionId']."---".$answeredCount);
                   $totalSurveyTakenUsers = count($scheduleObject->SurveyTakenUsers);
                   ...
                   $optionsPercentageArray = array();
                    foreach($question['OptionsNewArray'] as $key=>$value){
                     ...
                     // error_log($key."--------------------ssss---".$optionsPercentageArray[$key]);
                    }
                     $question['OptionsPercentageArray'] = $optionsPercentageArray;
...
//                        error_log("ifffffffffff");
     ...
     */
     public static function initiatePushNotification($obj,$main) {
        try {
          //  error_log("@@@@@ initiatePushNotification @@@@@@@@@@@---".$obj->ActionType."--userId---".$obj->UserId);
           //error_log("---orginaluserId----".$obj->OriginalUserId);
               ...
               if (isset($obj->ActionType) && $obj->ActionType == 'Mention') {
                   CommonUtility::sendPushNotification($obj,0);
                    ...
                    CommonUtility::sendPushNotification($obj,0);
                       ...
                       error_log("sendPushNotification-----------".$exc->getMessage());
    ...
    static function sendIOSPushNotification($message,$badge=1,$device_tokens,$actionType){
        ...
        //   error_log("##############paulaod".print_r($payload,true));
         ...
         // error_log("Network Name-----------------------".$networkName);
    ...
    static function findUrlInStringAndMakeLink($htmlString){
        try{
            ...
            $pattern1 = '/href=\"(.*?)\"/i';
            if (preg_match_all($pattern1, $htmlString, $matches)) {
                foreach ($matches[1] as $key => $matche) {
                    $parseurl = parse_url($matche);
                    if (!in_array("http", $parseurl) && !in_array("https", $parseurl)) {//not contains http and https  
                        $htmlString = str_replace('"' . $matche, '"http://' . $matche, $htmlString);
                    }
                }
            }
...
//}
 $title= $title."<i> $object->PostOn  </i>";
   $object->StreamTitle=$title;
         ...
         if($object->CategoryType == 8){
   
       
   $recentActivityUser1 = UserCollection::model()->getTinyUserCollection($loggedInUserId);
                $userId1 = "";
                ...
                $object->FirstUserId = $userId1;
                $object->FirstUserDisplayName = $displayName1;
              
                $object->FirstUserProfilePic = $recentActivityUser1['profile250x250'];               
                                    
               $object->PostId = $postId;
                        $object->StreamNote=$title;
...
static function decryptString($encrypted_string) {
    ...
    static function getGroupAccessibleUserStatusByGroupId($groupId,$lemail=""){
        try{
            ...
            error_log("########Exception occurred in getGroupAccessibleUserStatusByGroupId#######".$ex->getMessage());
             Yii::log($ex->getMessage(), "error", "application");
        }
    }
    static function downloadAfile($file,$folder){
        try{
            ...
            error_log("########Exception occurred in downloadAfile#######".$ex->getMessage());
             Yii::log($ex->getMessage(), "error", "application");
        }
    }
   ...
   public static   function getUserSaveItForLaterStream($userId,$condition,$type)
    ...
    static function prepateSurveyTakenUsersInfoData($surveyId, $scheduleId) {
        try {
            ...
            error_log("prepateSurveyTakenUsersInfoData---" . $ex->getMessage());
            Yii::log($ex->getMessage(), "error", "application");
        }
    }

    static function getAnswerForQuestion($questionId,$bufferAnswers){
       // error_log("getAnswerForQuestion----".$questionId);
        foreach ($bufferAnswers as $answer) {
            error_log("quesio-ddd---".$answer["QuestionId"]);
...
//));
    ...
    static function trackSurvey($userId, $scheduleId, $surveyId, $page, $type) {
        //error_log("trackSurvey---------------".$type."---page---".$page);
        try{
           if ($type == "refresh") {
            ...
            //  error_log("current page-----".$currentPage."---previos page----".$previousPage);
      ...
      Yii::log("-----in exception--------CommonUtility---------trackSurvey-----------".$ex->getMessage(), 'error', 'application');
        }
...
//            }
        ...
        try{
            ...
            if($isMine==1){
                if($actionedUserCount<=2){
                    $message = Yii::t('notifications',"{who} {action} your {postType} ".$News, $arr); 
                }else{
                    $message = Yii::t('notifications',"{who} and other {action} your {postType} ".$News, $arr);
                }
            }else{
                if($isAnonymous==1){
                    if($actionedUserCount<=2){
                        $message = Yii::t('notifications',"{who} {action} {postType} ".$News, $arr); 
                    }else{
                        $message = Yii::t('notifications',"{who} and other {action} {postType} ".$News, $arr);
                    }
                }else{
                    if($actionedUserCount<=2){
                        $message = Yii::t('notifications',"{who} {action} {whose}'s {postType} ".$News, $arr); 
                    }else{
                        $message = Yii::t('notifications',"{who} and other {action} {whose}'s {postType} ".$News, $arr);
                    }
                }
            }
            return $message;
        }catch(Exception $ex){
            error_log("========notificationStringForLove=======".$ex->getMessage());
        }
    }
    public static function notificationStringForComment($who, $action, $postType, $actionedUserCount, $isAnonymous=0, $isMine=0, $whose="", $hashtag=""){
        try{
            ...
            if($hashtag!=""){
                ...
                if($isMine==1){
                    if($actionedUserCount<=2){
                        $message = Yii::t('notifications',"{who} {action} on your {postType} ".$News, $arr); 
                    }else{
                        $message = Yii::t('notifications',"{who} and other {action} on your {postType} ".$News, $arr);
                    }
                }else{
                    if($isAnonymous==1){
                        $arr = array("{who}"=>$who,"{action}"=>$action, "{postType}"=>$postType);
                        if($actionedUserCount<=2){
                            $message = Yii::t('notifications',"{who} {action} on {postType} ".$News, $arr); 
                        }else{
                            $message = Yii::t('notifications',"{who} and other {action} on {postType} ".$News, $arr);
                        }
                    }else{
                        if($actionedUserCount<=2){
                            $message = Yii::t('notifications',"{who} {action} on {whose}'s {postType} ".$News, $arr); 
                        }else{
                            $message = Yii::t('notifications',"{who} and other {action} on {whose}'s {postType} ".$News, $arr);
                        }
                    }
                }
            }
            ...
            error_log("========notificationStringForComment=======".$ex->getMessage());
        }
    }
    public static function notificationStringForFollow($who, $action, $postType, $actionedUserCount, $isAnonymous=0, $isMine=0, $whose=""){
        try{
            ...
            if($isMine==1){
                if($actionedUserCount<2){
                    $message = Yii::t('notifications',"{who} is {action} your {postType} ".$News, $arr); 
                }else if($actionedUserCount==2){
                    $message = Yii::t('notifications',"{who} are {action} your {postType} ".$News, $arr); 
                }else{
                    $message = Yii::t('notifications',"{who} and other are {action} your {postType} ".$News, $arr);
                }
            }else{
                if($isAnonymous==1){
                    if($actionedUserCount<2){
                        $message = Yii::t('notifications',"{who} is {action} {postType} ".$News, $arr); 
                    }else if($actionedUserCount==2){
                        $message = Yii::t('notifications',"{who} are {action} {postType} ".$News, $arr); 
                    }else{
                        $message = Yii::t('notifications',"{who} and other are {action} {postType} ".$News, $arr);
                    }
                }else{
                    if($actionedUserCount<2){
                        $message = Yii::t('notifications',"{who} is {action} {whose}'s {postType} ".$News, $arr); 
                    }else if($actionedUserCount==2){
                        $message = Yii::t('notifications',"{who} are {action} {whose}'s {postType} ".$News, $arr); 
                    }else{
                        $message = Yii::t('notifications',"{who} and other are {action} {whose}'s {postType} ".$News, $arr);
                    }
                }
            }
            return $message;
        }catch(Exception $ex){
            error_log("========notificationStringForFollow=======".$ex->getMessage());
        }
    }
    public static function notificationStringForInvite($who, $action, $postType, $postTypeId){
        try{
            ...
            $arr = array("{who}"=>$who,"{action}"=>$action, "{postType}"=>$postType);
            if($postTypeId==2){
                $message = Yii::t('notifications',"{who} is {action} you to an {postType} ".$News, $arr);
            }else{
                $message = Yii::t('notifications',"{who} is {action} you to a {postType} ".$News, $arr);
            }
            return $message;
        }catch(Exception $ex){
            error_log("========notificationStringForInvite=======".$ex->getMessage());
        }
    }
    public static function notificationStringForMention($who, $action, $postType, $postTypeId){
        try{
            ...
            $arr = array("{who}"=>$who,"{action}"=>$action, "{postType}"=>$postType);
            if($postTypeId==2){
                if($postType=='Group Event'){
                     $message = Yii::t('notifications',"{who} {action} you in a {postType}", $arr); 
                }else{
                    $message = Yii::t('notifications',"{who} {action} you in an {postType}", $arr); 
                }
            ...
            error_log("========notificationStringForMention=======".$ex->getMessage());
        }
    }
    public static function notificationStringForUserFollow($who, $action, $actionedUserCount){
        try{
            ...
            error_log("========notificationStringForUserFollow=======".$ex->getMessage());
        }
    }
    public static function notificationStringForPost($who, $postType, $hashtag="", $curbsideCategory=""){
        try{
            ...
            error_log("========notificationStringForPost=======".$ex->getMessage());
        }
    }
    ...
    static function getTranslatedPostTypebyIndex($type, $isGroupCategory = 0) {
        try {
            ...
            Yii::log("===============getTranslatedPostTypebyIndex=============" . $exc->getMessage(), 'error', 'application');
        }
    }
    ...
    public static function streamNoteForNormalEventPost($recentActivity, $postType, $isMine=0, $invitedUsers="", $isInvited="", $isInteractions=0){
        try{
            ...
            if($recentActivity=="Post"){
                ...
                if(!$isInteractions){
                    $message = Yii::t('streamnotes',"invited you to an {postType}", $arr);
                }else if($isInvited){
                    if($isMine){
                        $message = Yii::t('streamnotes',"have been invited to an {postType}", $arr);
                    }else{
                        $message = Yii::t('streamnotes',"invited to an {postType}", $arr);
                    }
                }else{
                    if($isMine){
                        $message = Yii::t('streamnotes',"have been invited {invitedUsers} to an {postType}", $arr);
                    }else{
                        $message = Yii::t('streamnotes',"has been invited {invitedUsers} to an {postType}", $arr);
                    }
                }
            ...
            error_log("========streamNoteForNormalEventPost=======".$ex->getMessage());
        }
    }
    public static function streamNoteForGames($recentActivity, $postType){
        try{
            ...
            if($recentActivity=="GameSchedule" || $recentActivity=="Post"){
                ...
                $message = Yii::t('streamnotes',"played a {postType}", $arr);
            }
            return $message;
        }catch(Exception $ex){
            error_log("========streamNoteForGames=======".$ex->getMessage());
        }
    }
    public static function streamNoteForBadges($recentActivity, $badgeName){
        try{
            ...
            if($recentActivity=="Comment"){
                $message = Yii::t('streamnotes',"commented on the {badgeName}", $arr);
            }else{
                $message = Yii::t('streamnotes',"unlocked the {badgeName}", $arr);
            }
            return $message;
        }catch(Exception $ex){
            error_log("========streamNoteForBadges=======".$ex->getMessage());
        }
    }
    public static function streamNoteForCV($recentActivity, $postType, $cvType, $isMine=""){
        try{
            ...
            if($recentActivity=="Post"){
                if($isMine){
                    $message = Yii::t('streamnotes',"updated your {type} in CV", $arr);
                }else{
                ...
                $message = Yii::t('streamnotes',"invited you to their {type} in CV", $arr);
            }else if($recentActivity=="Love"){
                if($isMine){
            ...
            error_log("========streamNoteForCV=======".$ex->getMessage());
        }
    }
    public static function prepareStreamNotes($postTypeId, $categoryType, $recentActivity, $postType, $isAnonymous=0, $isMine=0, $invitedUsers="", $isInvited=0, $isInteractions=0){
        try{
            ...
            if($isAnonymous && $recentActivity=="Post"){//Anonymous Post
                $message = Yii::t('streamnotes',"A new {postType} has been created "."$News", $arr);
                ...
                $message = Yii::t('streamnotes',"commented on a {postType} " ."$News", $arr);
            }else if($recentActivity=="UserMention"){
                ...
                if(!$isInteractions){
                    if($postTypeId==2){
                        if($categoryType==3){
                             $message = Yii::t('streamnotes',"invited you to a {postType} "."$News", $arr);
                        }else{
                             $message = Yii::t('streamnotes',"invited you to an {postType} "."$News", $arr);
                        }
                    ...
                    if($isMine){
                        $message = Yii::t('streamnotes',"have been invited to a {postType} "."$News", $arr);
                    }else{
                       if($postTypeId==2){
                        if($categoryType==3){
                             $message = Yii::t('streamnotes',"invited you to a {postType} "."$News", $arr);
                    ...
                    if($isMine){
                        $message = Yii::t('streamnotes',"have invited {invitedUsers} to a {postType} "."$News", $arr);
                    }else{
                        $message = Yii::t('streamnotes',"has invited {invitedUsers} to a {postType} "."$News", $arr);
                    }
                }
            }
            else if($recentActivity=="SubGroupFollow"  || $recentActivity=="GroupFollow" || $recentActivity=="HashTagFollow" ||$recentActivity=="CurbsideCategoryFollow" || $recentActivity=="PostFollow" ){
                 
                  if($isMine){
                        $message = Yii::t('streamnotes',"are following a {postType} "."$News", $arr);
                    }else{
                         $message = Yii::t('streamnotes',"is following a {postType} "."$News", $arr);
                    }
            }
            ...
            }else if($postTypeId==3 ||$postTypeId==2){
                $message = Yii::t('streamnotes',"created a {postType} "."$News", $arr);
            }
            
            else{
                $message = Yii::t('streamnotes',"posted a {postType} "."$News", $arr);
            }
            return $message;
        }catch(Exception $ex){
            error_log("========prepareStreamNotes=======".$ex->getMessage());
        }
    }
    ...
    public static function getStreamNote($categoryType, $postTypeId, $recentActivity, $isAnonymous, $isMine=0, $badgeName = "", $cvType="", $invitedUsers="", $isInvited=0, $isInteractions=0){
        try{
            ...
            error_log("==============getStreamNotes==============".$ex->getMessage());
        }
    }
     /*
...
Yii::log("-----in exception--------CommonUtility---------calculateTimeSpentOnSurvey-----------".$ex->getMessage(), 'error', 'application');
        }
    }
...
Yii::log("-----in exception--------CommonUtility---------getSurveyPageAnalytics-----------".$ex->getMessage(), 'error', 'application');
        }
    }
    static function prepareJustificationData($surveyId,$questionId,$justificatonData,$questionType){
        try{
           // error_log("prepareJustificationData-----------------------".print_r($justificatonData,1));
           ...
           for($i=0;$i<count($usersArray);$i++){
               ...
               // error_log ("userId----------------------".$usersArray[$i]);
               //  error_log ("option commetns----------------------".print_r($optionCommentsArray[$i],1));
          ...
          //   error_log("--surveyUId------------------".$surveyId);
            ...
            error_log("optin anmes---------------".print_r($optionNames,1));
            //error_log("optnos cimm------".print_r($optionCommentsArray,true));
       //    error_log("-prepareJustificationData---".print_r($finalDataArray,1));
            return array("finalArray"=>$finalDataArray,"OptionName"=>$optionNames,"LabelName"=>$labelName);   
            
        } catch (Exception $ex) {
Yii::log("-----in exception--------CommonUtility---------prepareJustificationData-----------".$ex->getMessage(), 'error', 'application');
        }
    }
...
Yii::log("-----in exception--------CommonUtility---------prepareJustificationData-----------".$ex->getMessage(), 'error', 'application');
    ...
    public static function getNotificationTypeByName($name){
        try{
            ...
            error_log("==============getNotificationTypeByIndex==============".$ex->getMessage());
        }
    }
    ...
    //                    Notifications::model()->saveNotifications($notificationObj);
            ...
            error_log("==============sendNotificationsToUsers==============".$ex->getMessage());
        }
    }
...
static function getGeocodes($addressArray){
  ...
  if($flag != ""){
   ...
   Yii::log("-----in exception--------CommonUtility---------getGeocodes-----------".$ex->getMessage(), 'error', 'application'); 
}  
    }
    ...
    public static function checkJoyrIdeStatus($userId,$opportunityId,$joyrideInfoObj)
    {
        try
        {
             ...
             if(sizeof($opportunityObjResult)>0 && sizeof($userAchivementsObjResult)>0)
             {
                ...
                // error_log("return value opportunityobj_________".print_r($userOpportunitiesObj['Opportunities'][0],true));
             ...
             error_log("===============checkJoyrIdeStatus=============" . $ex->getMessage());
        }
    }
...
//               
            ...
            Yii::log("===============calculateGoalAtOpportunityLevel=============" . $exc->getMessage(), 'error', 'application');
        }
   ...
   Yii::log("===============triggerRecommendationsForUser=============" . $ex->getMessage(), 'error', 'application');
        }
    }
        ...
        try{
            ...
            if(isset($surveyUserIdsArray) && is_array($surveyUserIdsArray)){
                ...
                foreach($surveyUserIdsArray[$pageIndex] as $value){                
                   ...
                   if($value != ""){
                        ...
                        $userObj =  UserCollection::model()->getTinyUserCollection($value);
           ...
           return array("finalArray"=>$finalDataArray);   
                ...
                Yii::log("Exception in--CommonUtility--prepareSurveyUsersData---".$ex->getMessage(), 'error', 'application');
        }
    }
 ...
 public static function trackUserSession($sessionId,$activityType,$userId,$date=null){
        return TrackUserActiveSessions::model()->trackStartSession($sessionId,$activityType,$userId,$date);
    }
     public static function activateUserSession($sessionId){
        return TrackUserActiveSessions::model()->activateUserSession($sessionId);
    }
    public static function checkUserSession($sessionId){
        return TrackUserActiveSessions::model()->checkUserSession($sessionId);
    }
  public static function getPostType($postId,$categoryType){
     try{
      return UserStreamCollection::model()->getPostType($postId,$categoryType);    
     } catch (Exception $ex) {
  Yii::log("CommonUtility:getPostType::".$ex->getMessage()."--".$ex->getTraceAsString(), 'error', 'application');
     }
   ...
   * @return string
   */
   function constructDailyDigestPostURLs($postId,$categoryId,$firstUserId){
    // error_log("construct url-----------".$postId."---".$categoryId);
     switch ($categoryId) {
...
//            break;
        case "7":
            $categoryType = "/subgroups";
            break;
        case "8":
            $categoryType = "/newsdetail";
            break;
        case "9":
            $categoryType = "/games";
            $firstUserId = "detail";
      ...
      $url = Yii::app()->params['ServerURL'].$categoryType."/".$postId."/".$firstUserId;
      error_log($url);
      return $url;
    }
    /**
     ...
     * @return string
     */
public static function getURLString($categoryId){
  //alert('geturlstring----'+categoryId);
    $categoryType;
    switch ($categoryId) {
...
return $categoryType;
 ...
 public static function prepareResultObject($status, $message, $data = array(),$code='') {
