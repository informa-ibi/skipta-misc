...
class SkiptaPostService {
     ...
     */
    public function savePost($postObj, $hashTagArray, $isDevice=0) {        
        try {         
            ...
            $hashTagIdArray = $this->saveHashTags((int) $postObj->UserId, $hashTagArray, 'Post', $postType, $categoryId, '', $postObj->CreatedOn, $isDevice);
            $posturls = array();
             ...
             */
            if (!CommonUtility::prepareStreamObject($postObj->UserId, "Post", $postId, $categoryId, '', '','',$isDevice)) {
                return "failure";
            }
            if ($postObj->Type == 2) {
                if (!CommonUtility::prepareStreamObject((int) $postObj->UserId, "EventAttend", $postId, $categoryId, '', '', $postObj->CreatedOn,$isDevice)) {
                    return "failure";
                }
            }
            if ($isAbused) {
            ...
            if ($returnValue != 'failure') {
                if (sizeof($postObj->Mentions) > 0) {
                    foreach ($postObj->Mentions as $value) {
                        ...
                        UserInteractionCollection::model()->saveMentionUsageActivity($categoryId, $postObj->Type, $postObj->UserId, $value, $activityIndex, $activityContextIndex, $isDevice);
                    }
                }
            }
            if (isset($postObj->PostedBy) && !empty($postObj->PostedBy)) {
                $this->saveFollowOrUnfollowToPost($postType, $postId, $postObj->PostedBy, 'Follow', $categoryId,'',$isDevice);
            }
            if (isset($postObj->CreatedOn) && !empty($postObj->CreatedOn)) {
                $returnValue = $postId;
            }
     ...
     * @return type
     */
    public function saveComment($commentObj, $NetworkId, $CategoryType, $isDevice=0) {
        try {// throw new Exception('diveision errror');
            ...
            $hashTagIdArray = $this->saveHashTags((int) $commentObj->UserId, $hashTagArray, "Comment", $commentObj->PostType, $CategoryType,'', $commentObj->CreatedOn, $isDevice);
            //$userId, $hashTagArray, $from = '', $postType = '', $category = '', $id = "", $createdon = '', $isDevice
...
//     UserInteractionCollection::model()->savePostActionActivity($CategoryType,$commentObj->PostType,$commentObj->UserId,$activityIndex,$activityContextIndex);   
     ...
     * @return string
     */
    public function saveLoveToPost($postType, $postId, $userId, $categoryType, $createdDate = '', $isDevice=0) {
                  ...
                  //  error_log("artifacts---------".$artifact);
                   ...
                   // error_log("path---------".$path);
                    ...
                    if ($postType == 1 || $postType == 2 || $postType == 3 || $postType == 4 || $postType == 5) {
                        $path = '/upload/'.$project.'/public/'. $path;
                    } else {
                        $path = '/upload/'.$project.'/public/'. $path;
                    }
                    $thumbNailpath = '/upload/'.$project.'/public/';
            ...
            if ($userObj != 'true') {
               ...
               $groupId = GroupCollection::model()->saveNewGroup($groupObj,$UserId,$networkAdminUserId);               
                if ($groupId != 'failure') {
                    ...
                    UserInteractionCollection::model()->trackNewGroupCreation("GroupCreation",$UserId, $groupId, $activityIndex, $activityContextIndex, $NetworkId, $segmentId,"");
...
public function updateGroupObject($groupObj) {
    ...
    public function abusePost($postId, $actionType, $categoryId, $networkId, $userId, $isBlockedPost = 0) {
        try {
            error_log("postId---".$postId."---actiontupe--".$actionType."--categorId---".$categoryId."--neworkid---".$networkId."--useid--".$userId);
            $returnValue = "failure";
            $description = "";
            if ((int) $categoryId == 2) {
    ...
    public function cancelSaveitforlaterPost($postId, $userId,  $categoryId, $networkId,$postType) {
        try {
           ...
           try {
                ...
                Yii::log("in cancelSaveitforlaterPost service stream" . $ex->getMessage(), 'error', 'application');
                $returnValue = 'success';
            }
            return $returnValue;
        } catch (Exception $exc) {
                    ...
                    if ($hashtagObj == 'noHashTag') {
                        ...
                        foreach ($hashtagObj as $hash) {
                            if ($from == "Post") {
                                ...
                                UserInteractionCollection::model()->saveHashTagCreationActivity($from, $category, $postType, $userId, $hash->_id, $hash->HashTagName, $activityIndex, $activityContextIndex, $id, $createdon, $segmentId);
                            }
                            if ($from == "Group" || $from == "SubGroup") {
                                $activityIndex = CommonUtility::getUserActivityIndexByActionType("HashtagUsed");
    ...
    public function getPostsForSearch($searchText, $offset, $pageLength) {
        $posts= PostCollection::model()->getPostsForSearch($searchText, $offset, $pageLength);
        $curbsidePosts=CurbsidePostCollection::model()->getCurbPostsForSearch($searchText, $offset, $pageLength);        
        return array_merge($posts,$curbsidePosts);
    }
...
if($artifact!=""){    
                    ...
                    if ($category == 1 || $category == 2 || $category == 8 || $category == 9 || $category==10 || $category==12) {
                        ...
                        if ($postType == 1 || $postType == 2 || $postType == 3 || $postType == 4 || $postType == 5) {
                            $path = '/upload/'.$project.'/group/' . $path;
                        }
                    }
...
//                     $fileNameTosave=  str_replace("//", "/", $fileNameTosave);
    ...
    public function GetPostsBasedonDateRangeAndType($startDate, $endDate, $type, $NetworkId, $GameAvailable, $NewsAvailable, $NormalUsers, $segmentId = 0,$graphType="Post") {
...
//            $timezone = Yii::app()->session['timezone'];
            ...
            $Resultsid = array(
                 'year' => array('$year' => '$CreatedOn'),
                'week' => array("$modeType" => '$CreatedOn'),
            );
//------------------normal Posts Start-------------------------------------------------------------------------//
            
if ($graphType == "Post" || $graphType == "Curbside" || $graphType == "Event" ||  $graphType == "News"||  $graphType == "GroupPost" ||  $graphType == "MarketResearch" ||  $graphType == "Games"||  $graphType == "Jobs" ||  $graphType == "Survey"||  $graphType == "Featured"||  $graphType == "Promoted") {
    
    if ($graphType == "Post") {
    $userActivityContect=array('$in' => array(2));
    $CategoryType=array('$in' => array(1));
    $postType= array('$in' => array(1));
     $match = array(
                    "userActivityContext" => $userActivityContect,
     ...
     if ($graphType == "Curbside") {
    $userActivityContect=array('$in' => array(2));
    $CategoryType=array('$in' => array(2));
    $postType= array('$in' => array(5));
    ...
    $postType= array('$in' => array(3));
     ...
     $match = array(
         ...
         if ($graphType == "Featured") {
    $userActivityContect=array('$in' => array(70));
      ...
      $match = array(
         ...
         if ($graphType == "Promoted") {
    $userActivityContect=array('$in' => array(68));
         $match = array(
                    "userActivityContext" => $userActivityContect,
    ...
    $postType= array('$in' => array(11));
      ...
      if ($graphType == "Games") {
    $userActivityContect=array('$in' => array(132,2));
    $CategoryType=array('$in' => array(9));
      ...
      if ($graphType == "Jobs") {
    $userActivityContect=array('$in' => array(76));
    ...
    $postType= array('$in' => array(1,2,3));
     ...
     if ($graphType == "MarketResearch") {
    $userActivityContect=array('$in' => array(128));
      ...
      $match = array(
                ...
                if ($segmentId != 0) {
                 ...
                 if (is_array($NetworkId) && sizeof($NetworkId) > 0) {
                ...
                if (!empty($nresults) && $nresults != "") {
                    foreach ($nresults as $value) {
                         ...
                         if ($modeType == '$month') {
                            ...
                            $key = $value['_id']['year'] . '-' . $value['_id']['week'];
                        } else {
                            if ($value['_id']['week'] <= 9 && $modeType == '$week') {
                                $key = '0' . $value['_id']['week'];
                            } else {
                                $key = $value['_id']['week'];
                            }
                        }
                        
                        if (array_key_exists($key, $valid_times)) {

                            $existingArray[0] = $value['count'];
                            $finalArray[$key] = $existingArray;
                        } else {

                            $finalArray[$key] = array($value['count'], 0);
                        }
                    }
                }

                  if ($graphType == "Post") {
    $userActivityContect=array('$in' => array(16, 17, 18, 19,20, 21, 22, 15));
    $CategoryType=array('$in' => array(1));
    $postType= array('$in' => array(1));
     ...
     $match = array(
               ...
               if ($graphType == "Curbside") {
    $userActivityContect=array('$in' => array(16, 17, 18, 20, 21, 22, 15));
    $CategoryType=array('$in' => array(2));
    $postType= array('$in' => array(5));
    ...
    $postType= array('$in' => array(3));
     ...
     $match = array(
         ...
         if ($graphType == "Featured") {
                    $userActivityContect = array('$in' => array(16, 17, 18, 19, 20, 21, 22, 15, 25));

                    $match = array(
                        "userActivityContext" => $userActivityContect,
                        "userRelatedActivityContext" => array('$in' => array(70)),
                ...
                if ($graphType == "Promoted") {
                    ...
                    $match = array(
                        "userActivityContext" => $userActivityContect,
                        "userRelatedActivityContext" => array('$in' => array(68)),
    ...
    $CategoryType=array('$in' => array(9));
      ...
      if ($graphType == "Jobs") {
    $userActivityContect=array('$in' => array(75,73));
   ...
   // $postType= array('$in' => array(1,2,3));
     ...
     if ($graphType == "MarketResearch") {
    $userActivityContect=array('$in' => array(129,130));
      ...
      $match = array(
             ...
             //   }
              ...
              //  error_log("@@@@@@@@@@@" . print_r($cresults, true));
...
//                        }
         ...
         //   error_log("----------------------result array is ---------------" . print_r($resArray, true));
            return $resArray;
        } catch (Exception $e) {
    ...
    public function getEngagementAnalytics($startDate, $endDate, $type, $NetworkId, $GameAvailable, $NewsAvailable,$NormalUsers,$segmentId=0,$EngagemenType) {
         
        return UserInteractionCollection::model()->getNewStreamEngagement($startDate, $endDate, $type, $NetworkId, $GameAvailable, $NewsAvailable,$NormalUsers,$segmentId=0,$EngagemenType);
    }
        ...
        try {
         if ($from == "SetUpPassword") {
            ...
            $result = UserInteractionCollection::model()->trackPageLoad($from, $dataId, $userId, $activityIndex, $activityContextIndex, $NetworkId);
             ...
             Yii::log(" in UpdateRDSUserAcceskeyWithUserid".$ex->getMessage(), "error", "application");
        }
        return $result;
    }
  /*
    ...
    */
    ...
    public function getNewUserJoyrideDetailsByOpportunityId($userId,$opportunityId)
    {
        ...
        try {
                    ...
                    error_log($opportunityToLoad."________KKK___________".print_r($joyrideInfoObj,true));
    ...
    public function getUserJoyrideDetailsStatus($userId)
    {
        ...
        try{
             ...
             error_log("____________getUserJoyrideDetailsStatus___________________".sizeof($result));
    ...
    public function saveOrUpdateUserTourGuideState($userId,$opportunityId,$joyRideDivId)
    {
        try
        {
                    ...
                    if(sizeof($tourOpportunityObj)>0)
                    {
                       ...
                       if($joyrideInfoObj!="failure" && sizeof($joyrideInfoObj)>0)
                       {
                        //Here check if the check point has any goal to acheive check column GoalToReach is greatere than 0/
                         ...
                         error_log("Goal status____________________".$goalStatus);
                         if($goalStatus)
                             $status=0;
                         else
                             $status=1;
                ...
                Yii::log("Exception Occurred while executing saveOrUpdateUserTourGuideState  in SkiptaPostService==" . $ex->getMessage(), 'error', 'application');
        }
...
//            {
            ...
            //}
             ...
             $returnValue=$joyrideInfo;
                ...
                Yii::log("Exception Occurred while executing getJoyrideDetailsByOpportunityId  in SkiptaPostService==" . $ex->getMessage(), 'error', 'application');
        }
         return $returnValue;
    ...
    public function updateTourGuideStatusByUserId($userId)
    {
        try
        {
             ...
             Yii::log("Exception Occurred while executing updateTourGuideStatusByUserId  in SkiptaPostService==" . $ex->getMessage(), 'error', 'application');
        }
    }
 ...
 * @param type $categoryType
 */
public function latestNewsDetailPage($postId,$postType,$categoryType){
     try {  
...
//            $object = ServiceFactory::getSkiptaPostServiceInstance()->getNewsObjectById($postId);
            ...
            if((int)trim($categoryType)==8 && (int)trim($postType)==11){
     ...
     return array("data" => $object, "tinyObject" => $tinyUserProfileObject, "categoryType" => $categoryType,'commentsdata'=>$MoreCommentsArray,'IsCommented'=>$IsUserCommented,"lovefollowArray"=>$lovefollowArray,"canMarkAsAbuse"=>$canMarkAsAbuse);
        ...
        //ServiceFactory::getSkiptaPostServiceInstance()->trackEngagementAction($userId,"Post","PostDetailOpen",$postId,$categoryType,$postType);
            ...
            Yii::log("SkiptaPostService:latestNewsDetailPage::".$ex->getMessage()."--".$ex->getTraceAsString(), 'error', 'application');
        }
}
 ...
 * @param type $categoryType
 */
public function latestGameDetailPage($postId,$postType,$categoryType,$mode){
     try {
         //error_log('mode---------------------'.$mode);  
...
//            }
        ...
        if (is_array($UserPrivileges)) {
         ...
         return array("gameDetails"=>$gameDetailsArray[0],"gameBean"=>$gameDetailsArray[1],"mode"=>$mode,'commentsdata'=>$MoreCommentsArray,'IsCommented'=>$IsUserCommented,"lovefollowArray"=>$lovefollowArray,"canMarkAsAbuse"=>$canMarkAsAbuse,'userLanguage'=>Yii::app()->session['language']);
