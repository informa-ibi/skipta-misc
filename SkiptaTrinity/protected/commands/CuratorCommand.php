...
public function actionCurable()
    {
     ...
     foreach($curablePosts as $key){
        ...
        if($data=='')
        {
            ...
            $CNID=ServiceFactory::getSkiptaPostServiceInstance()->saveCuratedPost($CuratedContentDataPrepared);
            if(Yii::app()->params['AutoReleaseNews']=='ON'){
            if(isset($CNID)){
            $ReleasedBy=(int)53;
            $data = ServiceFactory::getSkiptaPostServiceInstance()->manageCuratedPostService($CNID, 1,$ReleasedBy);
            ServiceFactory::getSkiptaPostServiceInstance()->releaseNewsObjectToStream($CNID, $ReleasedBy);
            }
            }
