...
<!--     <div class="span10 "><h2 class="pagetitle"><?php //echo $data->TopicName?></h2></div>-->
  <ul class="breadcrumb">
      <li class="detailed_close_page1"><a href="#" id="breadcrumSoureEle"></a> <span class="divider">/</span></li>
      <li class="active">Detail</li>
    </ul>


<div class="post item <?php echo (isset($data->IsPromoted) && $data->IsPromoted==1)?'promoted':''; 
echo (isset($data->IsSaveItForLater) && $data->IsSaveItForLater==1) ?' saveitforlater' :'' ?>" style="width:100%;" id="postitem_<?php echo $data->_id; ?>" data-postid="<?php echo $data->PostId; ?>" data-posttype="<?php  echo $data->PostType;?>" data-categorytype="<?php  echo $data->CategoryType;?>">

<div class="stream_widget marginT10 positionrelative" >
    <?php include Yii::app()->basePath . '/views/post/stream_profile_image.php'; ?>
    ...
    <?php include Yii::app()->basePath . '/views/post/stream_header.php'; ?>
            
<!-- spinner -->
<div id="stream_view_spinner_<?php echo $data->_id; ?>"></div>
<div id="stream_view_detailed_spinner_<?php echo $data->PostId; ?>"></div>
<!-- end spinner -->
<div class=" stream_content positionrelative" style="padding-top:0">
...
<div class="customwidget <?php echo $data->Alignment?> customwidgetdetail" style="border:0">
    
    <?php if($data->Editorial!=''){?>
    <div  style="padding: 0px 10px">
<div class="row-fluid">
<div class="span12">
<div  class="decoratedNews span12 EDCRO<?php echo $data->_id?>"><?php echo $data->Editorial?></div>
</div>
</div>
        </div>
<?php }?>
...
if(breadCumSource != ""){
                     sessionStorage.setActiveClassPage = setActiveClassPage;
                     sessionStorage.breadCumSource = breadCumSource;
                       $("#"+setActiveClassPage).removeClass("active").addClass("active");
                }else{
                  
                     $("#"+sessionStorage.setActiveClassPage).removeClass("active").addClass("active");
                }
                  $("#contentDiv").removeClass("streamsectionarearightpanel");
            prepareBreadCrum();
            handlingbacknavigation();
             function prepareBreadCrum(){
       
          var url;
          $("#breadcrumSoureEle").html(sessionStorage.breadCumSource);
         // alert("breadCumSource---"+breadCumSource);
         if(breadCumSource == ""){ 
                      $("#breadcrumSoureEle").html("News");
                      
                      $(".detailed_close_page1").unbind("click").bind("click",function(){
                             window.location = "/news/index";
                         })
                     }else{
                          $( ".detailed_close_page1" ).unbind( "click" );
        $(".detailed_close_page1").bind('click',function(){ 
             $("#detail_rightpanel").hide();
           window.history.pushState("object or string", "Title","/"+sessionStorage.prevURI);
             if(sessionStorage.rightPanelExist == "true"){
          // $("#contentDiv").addClass("streamsectionarearightpanel");
            if($("#admin_PostDetails").length>0){  
                $("#admin_PostDetails").addClass("streamsectionarearightpanel");
            } 
            if($("#contentDiv").length>0){  
             ...
             if($("#admin_PostDetails").length>0){  
                $("#admin_PostDetails").removeClass("streamsectionarearightpanel");
            } 
            if($("#contentDiv").length>0){  
                $("#contentDiv").removeClass("streamsectionarearightpanel");
            } 
