<?php if(isset($userFollowers) && sizeof($userFollowers)>0){?>
       
                <div class="row-fluid " id="userFollowUnfollowid">

                    <?php $i=0; foreach ($userFollowers as $user){ ?> 
                    <?php if($i == 0){ ?>
                  ...
                  <div class="media-body">
                      ...
                      <div class="m_title profileDetails" style="cursor: pointer" id='<?php echo $user->UserId ?>' data-name='<?php echo $user->uniqueHandle ?>'><?php echo $user->DisplayName?></div>
     ...
     $('a.nonNetworkAdmin .follow').live("click",function(){        
           var userId=$(this).closest('a.userId').attr('data-id');           
             ...
             $("#userFollowunfollowa_" + userId).attr({"class": "unfollow"}); 
              $("#userFollowunfollowa_" + userId).attr({"data-original-title": "<?php echo Yii::t('translation','Follow'); ?>"});
              })
            ...
            $("#userFollowunfollowa_" + userId).attr({"class": "follow"});
             $("#userFollowunfollowa_" + userId).attr({"data-original-title": "<?php echo Yii::t('translation','UnFollow'); ?>"});
