<?php   $PostId=empty($PostId)?$data->_id :$PostId;
        ...
        if(!isset($streamType)){
   if (($data->CategoryType != 3 || $data->IsIFrameMode == 1) && $data->IsAbused != 1 && $isPostManagement==0  &&  $data->CategoryType != 5 && $data->CategoryType != 6 && $data->CategoryType != 10 && $data->PostType != 15 && $data->CategoryType != 11) { ?>
...
<script type="text/javascript">
   ...
   var queryString = "postId="+postId+"&categoryType="+categoryType+"&networkId="+networkId+"&type="+type; 
    ...
    $(" .PostManagementActions a.promote").live("click touchstart", function() {
     ...
     $("#postDetailedwidget .PostManagementActions a.copyurl, #admin_PostDetails .PostManagementActions a.copyurl").die().live("click", function() {
    ...
    $(".CommentManagementActions a.abuse").die().live("click touchstart", function() { 
        var modelType = 'error_modal';
        var title = 'Flag as Abuse';
        var content = "Are you sure you want to flag this message as inappropriate?";
        var closeButtonText = 'No';
