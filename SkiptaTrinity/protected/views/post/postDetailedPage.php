...
if(is_object($data)){
    ...
    if(($isPostManagement==0 && $data->IsAbused == 1 &&   Yii::app()->session['IsAdmin']!='1') || $data->IsDeleted == 1 ||  $data->IsDeleted == 2 || $data->IsBlockedWordExist == 1){ 
         $postText = CommonUtility::postStringTypebyIndex($data->Type); 
        ?>
        <div class="row-fluid">
            <div class="span10" style="text-align:center;font-family:'exo_2.0medium'">
                <h3> <?php echo "This ".$postText." ".$text; ?>.
...
<div class="row-fluid " id="postDetailedTitle">
     <h2 class="pagetitle"><?php if($data->Type==5){ echo $data->Subject;} else{  echo Yii::t('translation','Normal_Post_Detail');}?></h2>
    
    <ul class="breadcrumb">
      <li class="detailed_close_page"><a href="#" id="breadcrumSoureEle"></a> <span class="divider">/</span></li>
...
//    var PreviousCurrentArtifactpage=0;
   if(!detectDevices())
    ...
    <?php if ($this->whichmenuactive == 1) {?>
      ...
      // alert(sessionStorage.breadCumSource);
          var url;
          $("#breadcrumSoureEle").html(sessionStorage.breadCumSource);
         // alert(breadCumSource);
         
         if(breadCumSource == ""){
                          if("<?php echo $data->CategoryType?>" == "1"){
                              $("#breadcrumSoureEle").html("Home");
                              url = "/stream";
                         }else if("<?php echo $data->CategoryType?>" == "2"){
                              $("#breadcrumSoureEle").html("Curbside");
                              url = "/curbsideConsult";
                         }else if("<?php echo $data->CategoryType?>" == "3"){
                              $("#breadcrumSoureEle").html("Groups");
                              url = "/groups";
                         }
                         else if("<?php echo $data->CategoryType?>" == "7"){
                              $("#breadcrumSoureEle").html("Sub Group");
                              url = "/"+sessionStorage.prevURI;
                         }
                     
                         $(".detailed_close_page").unbind("click").bind("click",function(){
                             window.location = url;
                         })
                     }else{
