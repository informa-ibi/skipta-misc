...
//                                scrollPleaseWaitClose('locationUsability_Reports'); 
                    ...
                    var map, pointarray, heatmap;
                     var result = [];               
                     for(var i in data2.latitude){
                     
                        result.push(new google.maps.LatLng(data2.latitude[i],data2.longitude[i]));
                    }
                    var mapOptions = {
                        zoom: 4,
                    ...
                    map = new google.maps.Map(document.getElementById('locationchart_div'),
                            mapOptions);

                    var pointArray = new google.maps.MVCArray(result);
                ...
                setTimeout(function(){
                  convertasbinaryimage('locationchart_div', 'LocationUsability');
                }, 4000);
...
function openUsabilityActivitypdf(obj,analyticType){
 ...
 function convertasbinaryimage(divId,analyticType){
    html2canvas(document.getElementById(divId), {
        ...
        onrendered: function (canvas) {
            ...
            var queryString = "imgData=" + finalImageSrc + "&analyticsType="+analyticType;
            ajaxRequest('/analytics/AnalyticsSaveImageFromBase64', queryString, function (data) {
                saveImageFromBase64handler(data);
            });
 ...
 function saveImageFromBase64handler (data1){
 }
