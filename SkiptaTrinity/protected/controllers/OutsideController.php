...
//                    $scheduleId = "5462f372b96c3de22a8b4567";
                   // error_log("actionRenderQuestionView----------------1");
                    ...
                    if($schedulePatternArr[0] != "notscheduled"){
                         // error_log("actionRenderQuestionView----------------2");
                        $surveyId = $schedulePatternArr[2];
...
//                         } else{
                           ...
                           //   error_log("page---".$page."----i--".$surveyObjArray["i"]."--flag----".$surveyObjArray["flag"]);                              
                        ...
                        $obj = ServiceFactory::getSkiptaExSurveyServiceInstance()->getScheduleSurveyById("Id",$scheduleId);
                         ...
                         if($surveyObj->IsAcceptUserInfo == 1 && $reValue == 0){
                             ...
                             if( $surveyObj->IsAnalyticsShown == 0 || $obj->ShowThankYou == 1 ){
                                if(isset($obj->_id))
                                      ServiceFactory::getSkiptaExSurveyServiceInstance()->updateThanksFlagId($obj->_id);
    ...
    public function actionSureyQuestionPagination(){
        try{
            ...
            error_log("actionSureyQuestionPagination Exception---".$ex->getMessage());
        } 
     }

    public function actionValidateSurveyAnswersQuestion(){
        try{         
//            error_log("actionValidateSurveyAnswersQuestion--------------");
...
//                            }
                            ...
                            if($key == "AnyOtherComment"){
                                ...
                                //error_log("===1111111==sizeof========options=====".sizeof($widget34->Options));
...
////                                                $widget34->OptionOtherTextValue = $m;                                                  
                            ...
                            if($key == "OptionMatrixValueOther"){
                                $k=1;
                                if(sizeof($value)>0){ 
                                    //error_log("=====optionOtherTextA====".print_r($value,1));
                                    $OptionOtherTextA = array();                                    
                                    $uKey = 0;
                                    $vstr = "";                                    
                                    foreach($value as $ky=>$v){
...
////                                                $widget34->OptionOtherTextValue = $m;                                                  
                           ...
                           //error_log("=====anyother values===options====".print_r($widget34->Options[$j-1],1));
                    ...
                    if(isset( $_REQUEST["fromAutoSave"])){
                     ...
                     if($exsurveyObj->IsAcceptUserInfo == 1 && $reValue == 0){
                        $this->renderUserAcceptionInfo($surveyObject->SurveyId, $QuestionsSurveyForm->ScheduleId, $exsurveyObj->SurveyTitle);
                         ...
                         if($surveyObject->ShowThankYou == 1 || $exsurveyObj->IsAnalyticsShown == 0){
                            $this->renderPartial('thankyou',array('result'=>$surveyObject,"ScheduleId"=>$QuestionsSurveyForm->ScheduleId,"IsAnalyticsShown"=>$exsurveyObj->IsAnalyticsShown));
                        }else{
...
//                   error_log("SurveyTitle===$QuestionsSurveyForm->SurveyTitle======inside======if post======Is Mandatory==$QuestionsSurveyForm->IsMadatory");
//                   foreach($QuestionsSurveyForm->IsMadatory as $row){
//                       error_log("========is Mandatory=====value====$row");
//                   }
    ...
    public function actionCaptureUserInfo(){
        try{
            ...
            $reValue = ServiceFactory::getSkiptaExSurveyServiceInstance()->getProfileDetails($UserId,$userInfo->ScheduleId,$userInfo->SurveyId);
                error_log("====return Value===$reValue");
            if( $reValue == 0){
...
//                    $this->render('captureUserInfo',array("UserInfo"=>$userInfo,"displayName"=>$displayName,"profilePic"=>$profilePic));
    ...
    public function actionRenderStates(){
        try{
          ...
          //  error_log("statesi-----------".print_r($states, true));
    ...
    public function renderUserAcceptionInfo($surveyId,$scheduleId,$surveyTitle){ 
        try{
            ...
            if($return != "failure" && sizeof($return)>0){                
                ...
                $userInfo->NPINumber = $return->NPINumber;                
    ...
    public function actionLogoutSurveyPage(){
        error_log("actionLogoutSurveyPage------");
          $page = $_REQUEST['Page'];
