...
public function actionGetUserRecommendationsList() {
    try {
        ...
        Yii::log("----exception in actionGetUserRecommendationsList user controller-----" . $ex->getMessage(), 'error', 'application');
    }
}

    public function actionProfileDetails(){
    try {
          error_log("=====actionProfileDetails====userProfileId==========="); 
...
//        }
       ...
       if($loggedInUserId==$userProfileId){
        ...
        error_log("=====actionProfileDetails====userProfileId===========$userProfileId");
        $data = ServiceFactory::getSkiptaUserServiceInstance()->getUserProfileDetails($userProfileId,$loggedInUserId); 

        error_log("_______________1111111111_____________________");
      ...
      //  $displayName=$userProfileDetails->DisplayName;
        error_log("___________222222222222222_____________________");
       ...
       // $userFollowingSubGroups=ServiceFactory::getSkiptaUserServiceInstance()->getUserFollowingSubGroupsData($userProfileId,$loggedInUserId);        
        error_log("__________3333333333333333_____________________");
        ...
        error_log("__________44444444444444444444_____________________");
      
        $userCVDetails=ServiceFactory::getSkiptaUserServiceInstance()->getUserCVDetails($userProfileId);
       error_log("__________555555555555555555_____________________");
        ...
        $pDis=1;
        error_log("_________666666666666666____________________");
  ...
  public function actionGetUserSignedUpEvents()
{
    try
    {
            ...
            $segmentId = (int)$this->tinyObject['SegmentId'];
             $userEventsActivityList = ServiceFactory::getSkiptaUserServiceInstance()->getUserAttendingEventsForRightWidget($userid,$CurrentLoginDate,$segmentId);
           $EventsArray=array();
                ...
                foreach ($userEventsActivityList as $key => $value) {
                    ...
                    if($value->Language!=$language){
                     ...
                     $EventBean = new UserEventAttendBean();
                      ...
                      if($value->CategoryType==3){
                        $UserFollowingGroups=  ServiceFactory::getSkiptaUserServiceInstance()->groupsUserNotFollowing($userid);
                        if(is_array($UserFollowingGroups,$value->GroupId)){
                            
                        }else{
                              unset($userEventsActivityList[$key]);
                              continue;
                        }
                      }
    ...
    public function actionCheckSession(){
       ...
       if (Yii::app()->user->isGuest) {
               ...
               if($this->autoLogin == "true"){
                    $result = array("code"=>201,"status"=>"autosession"); 
               }else{
...
try{
     ...
     $this->render('settings',array('UserRegistrationForm'=> $UserSettingsForm,'ChangePasswordForm'=> $changePasswordForm,$countryId=>$countryId,"stateId"=>$data["State"],'countryRequest'=>$CountryRequest, "message"=>$message,'CustomForm'=>$customForm,'CustomObject'=>$customObject,$countryId=>$countryId,"stateId"=>$data["State"],'subSpe'=>$subSpe));
  ...
  public function actionUpdateUserSettings(){
                ...
                if ($errorUserSettingsForm != "[]") {
                   ...
                   $UserSettingsForm['email']=trim($UserSettingsForm['email']);
                    ...
                    if (count($userexist) > 0) {
                        ...
                        error_log("_________**********___________________________".$UserSettingsForm['country']);
                        $updateCountry= $oldUserObj['Country'] != $UserSettingsForm['country']?false:true;
                        $Save_userInUserCollection = ServiceFactory::getSkiptaUserServiceInstance()->UpdateUserCollection($UserSettingsForm,$oldUserObj,$updateCountry);       
                        if ($Save_userInUserCollection != 'error') {
                            if ($oldUserObj['Country'] != $UserSettingsForm['country']) {
                                ...
                                $status = ServiceFactory::getSkiptaUserServiceInstance()->requestForChangeCountry($prefenceform->attributes, $this->tinyObject->UserId);
                                if ($status == "success") {
                            ...
                            $obj = array('status' => 'success', 'tinyObject'=>$tinyUserCollectionObj,'data' => $message.$message1, 'error' => '');
                        } else {
     ...
     public function actionDynamicstates() {
        ...
        if(isset($_POST['mobile']) && !empty($_POST['mobile'])){
            ...
            if (count($data) > 0) {
                $data = CHtml::listData($data, 'StateCode', 'State');
    ...
    public function actionGetActionUsers(){
        try{
           ...
           $page = $_REQUEST['page'];
           $pageSize = ($pageLength * $page); 
            error_log("postid-------------------".$id."--actionTupe---".$actionType."---userId---".$userId."--pagesize-----".$pageSize."---pagelength---".$pageLength."---flag---".$flag);
...
//update the language and source language in session
    ...
    public function actionChangeSegmentPageLoad() {
        try {
            ...
            Yii::log("------Exception Occurred in UserController/actionChangeSegmentPageLoad==" . $ex->getMessage(), "error", "application");
        }
    }
...
public function actionTrackUserSession(){
    error_log("--actionTrackUserSession-------");
     $sessionId = $_POST["sessionId"];
     $UserId = $_POST["UserId"];
     $date = $_POST["date"];
     $activityType = $_POST["activityType"];
    
      $userSessionId = CommonUtility::trackUserSession($sessionId,$activityType,$UserId,$date);
}
public function actionActivateUserSession(){
    
     $sessionId = $_POST["sessionId"];
      $userSessionId = CommonUtility::activateUserSession($sessionId);
}
