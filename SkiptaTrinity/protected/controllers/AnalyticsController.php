...
//exit;
    ...
    public function actiongetActivityDetails(){      
        try {
            ...
            if (isset($request['startDate']) && $request['startDate'] != 'undefined') {
                $startDate = DateTime::createFromFormat(Yii::app()->params['PHPDateFormat'], $request['startDate'])->format("Y-m-d");
            } else {
                $startDate = date('Y-m-d', strtotime("-7 days"));
            }
            if (isset($request['endDate']) && $request['endDate'] != 'undefined') {
                $endDate = DateTime::createFromFormat(Yii::app()->params['PHPDateFormat'], $request['endDate'])->format("Y-m-d");
            } else {
            ...
            $NetworkId = $request['networkId'];
             if(isset($NetworkId) && $NetworkId!="" && strlen($NetworkId)>0 && $NetworkId!='0'){
                 $NetworkId=array_map('intval', explode(',', $NetworkId));
                //$NetworkId=explode(",",$NetworkId);
            }
            // error_log('networkId'.$NetworkId);
            // $NetworkId = 16;
            $xyArray = array();
            $NormalUsers = array();
//            $NormalUsersList = ServiceFactory::getSkiptaPostServiceInstance()->getTechAndBusinesUserIds($NetworkId);
...
//        }
            ...
            if (Yii::app()->params['IsDSN'] == 'ON') {
                if ($GameAvailable == 'ON' && $NewsAvailable == 'ON') {
                    $xyArray = array('Year', 'Conversations', 'Events', 'Poll', 'Featured Items', 'Promoted Posts', 'Active Users', 'Comeback Users', 'Registrations', 'News', 'Games', 'Jobs');
                } else if ($GameAvailable == 'ON' && $NewsAvailable == 'OFF') {
                    $xyArray = array('Year', 'Conversations', 'Events', 'Poll', 'Featured Items', 'Promoted Posts', 'Active Users', 'Comeback Users', 'Registrations', 'Games', 'Jobs');
                } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'ON') {
                    $xyArray = array('Year', 'Conversations', 'Events', 'Poll', 'Featured Items', 'Promoted Posts', 'Active Users', 'Comeback Users', 'Registrations', 'News', 'Jobs');
                } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'OFF') {
                    $xyArray = array('Year', 'Conversations', 'Events', 'Poll', 'Featured Items', 'Promoted Posts', 'Active Users', 'Comeback Users', 'Registrations', 'Jobs');
                }

                $graphArray = ServiceFactory::getSkiptaPostServiceInstance()->GetPostsBasedonDateRangeAndType($startDate, $endDate, 'graph', $NetworkId=0, $GameAvailable, $NewsAvailable, $NormalUsers, $segmentId=0);
                ...
                if ($GameAvailable == 'ON' && $NewsAvailable == 'ON') {
                    $xyArray = array('Year', 'Posts', 'Curbside Consults', 'Events', 'Poll', 'Featured Items', 'Promoted Posts', 'Active Users', 'Comeback Users', 'Registrations', 'News', 'Games', 'Jobs');
                } else if ($GameAvailable == 'ON' && $NewsAvailable == 'OFF') {
                    $xyArray = array('Year', 'Posts', 'Curbside Consults', 'Events', 'Poll', 'Featured Items', 'Promoted Posts', 'Active Users', 'Comeback Users', 'Registrations', 'Games', 'Jobs');
                } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'ON') {
                    $xyArray = array('Year', 'Posts', 'Curbside Consults', 'Events', 'Poll', 'Featured Items', 'Promoted Posts', 'Active Users', 'Comeback Users', 'Registrations', 'News', 'Jobs');
                } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'OFF') {
                    $xyArray = array('Year', 'Posts');
                }
                 $xyArray = array('Year', 'Created','Engaged');
               
               //    error_log("*************".$startDate."E".$endDate."N".$NetworkId."GAM".$GameAvailable."New".$NewsAvailable."NU".$NormalUsers."SEG".$segmentId."Grapth".$graphType);
                $graphArray = ServiceFactory::getSkiptaPostServiceInstance()->GetPostsBasedonDateRangeAndType($startDate, $endDate, 'graph', $NetworkId=0, $GameAvailable, $NewsAvailable, $NormalUsers, $segmentId=0,$graphType);
            }
            ...
            $status = "success";
            $message = "success";
            $code = Yii::t('errorcodes', 'Success');
            // $result="tess";

            $result = array("graphicArray" => $graphArray, "heighestValue" => $heighestValue, 'lablesArray' => $xyArray);
           // $result= CommonUtility::prepareResultObject($status, $message, $result, $code);
            $result = array("json"=>  json_encode($graphArray),"heighestValue"=>$heighestValue,'lablesArray'=>$xyArray);  
  ...
  public function actionGetEngagementDetails(){
    ...
    try{
      ...
      $endDate = $endDate." 23:59:00";
        ...
        if (isset($_POST['divId']) && $_POST['divId'] != 'undefined' && $_POST['divId'] != '') {
                $EngagemenType=$_POST['divId'];
            }
     
     $graphArray=ServiceFactory::getSkiptaPostServiceInstance()->getEngagementAnalytics($startDate,$endDate,'graph',$NetworkId,$GameAvailable,$NewsAvailable,$NormalUsers,$segmentId,$EngagemenType);
...
//      {
//         if($GameAvailable=='ON' && $NewsAvailable=='ON'){
//           $xyArray = array('Year', 'Stream','Posts','Named Post','Events','Surveys','Groups','Hashtags','News','Games'); 
//        }else if($GameAvailable=='ON' && $NewsAvailable=='OFF'){
//            $xyArray = array('Year', 'Stream','Posts','Named Post','Events','Surveys','Groups','Hashtags','Games'); 
//        }else if($GameAvailable=='OFF' && $NewsAvailable=='ON'){
//            $xyArray = array('Year', 'Stream','Posts','Named Post','Events','Surveys','Groups','Hashtags','News'); 
//        }else if($GameAvailable=='OFF' && $NewsAvailable=='OFF'){
//             $xyArray = array('Year', 'Stream','Posts','Named Post','Events','Surveys','Groups','Hashtags'); 
//        }       
////       $startDate = CommonUtility::convert_time_zone(strtotime($startDate),  "PST",Yii::app()->session['timezone']);
////       $endDate =  CommonUtility::convert_time_zone(strtotime($endDate),"PST",Yii::app()->session['timezone']);
//     $graphArray=ServiceFactory::getSkiptaPostServiceInstance()->getEngagementAnalytics($startDate,$endDate,'graph',$NetworkId,$GameAvailable,$NewsAvailable,$NormalUsers,$segmentId,$EngagemenType);
...
//        $heighestValue = CommonUtility::get_highest($graphArray);
     //$result = array("json"=>  json_encode($graphArray),"heighestValue"=>$heighestValue,'lablesArray'=>$xyArray);
      $result = array("result" => $graphArray, 'lablesArray' => $xyArray);
        echo $this->rendering($result); 
    } catch (Exception $ex) {
    ...
    public function actionAnalyticsSaveImageFromBase64(){
     ...
     public function actionActivityGenerateXLS(){
          ...
          //Some data
            ...
            $featuredItems = ServiceFactory::getSkiptaPostServiceInstance()->GetPostsBasedonDateRangeAndType($startDate,$endDate,'report',$NetworkId,$GameAvailable,$NewsAvailable,$NormalUsers,$segmentId);  
             ...
             if(Yii::app()->params['IsDSN']=='ON')
               ...
               if ($GameAvailable == 'ON' && $NewsAvailable == 'ON') {
                                $ActivityColumnsArray = array('Date','Conversations', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations', 'News', 'Games','Jobs');
                            } else if ($GameAvailable == 'ON' && $NewsAvailable == 'OFF') {
                                $ActivityColumnsArray = array('Date','Conversations', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations', 'Games','Jobs');
                            } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'ON') {
                                $ActivityColumnsArray = array('Date','Conversations', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations', 'News','Jobs');
                            } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'OFF') {
                                $ActivityColumnsArray = array('Date','Conversations', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations','Jobs');
                            }
       ...
       {  if ($GameAvailable == 'ON' && $NewsAvailable == 'ON') {
                $ActivityColumnsArray = array('Date', 'Posts', 'Named Post', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations', 'News', 'Games','Jobs');
            } else if ($GameAvailable == 'ON' && $NewsAvailable == 'OFF') {
                $ActivityColumnsArray = array('Date', 'Posts', 'Named Post', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations', 'Games','Jobs');
            } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'ON') {
                $ActivityColumnsArray = array('Date', 'Posts', 'Named Post', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations', 'News','Jobs');
            } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'OFF') {
                $ActivityColumnsArray = array('Date', 'Posts', 'Named Post', 'Events', 'Surveys', 'FeaturedItems', 'PromotedPosts', 'ActiveUsers', 'ComebackUsers', 'Registrations','Jobs');
            }
        ...
        try{   
          ...
          $dateFormat = CommonUtility::getDateFormat();
            ...
            // $featuredItems = ServiceFactory::getSkiptaPostServiceInstance()->GetPostsBasedonDateRangeAndType($startDate,$endDate,'report',$NetworkId);  
              ...
              {
                if($GameAvailable=='ON'){
               $EngagementColumnsArray=array('Date','Stream','Conversations','EventPosts','SurveyPosts','Groups','Hashtags','News','Compete');
            }else{
        ...
        foreach ($graphArray as $key => $value) {
           ...
           // $data[$i][$j+8]=$value[$j+7];
            ...
            $i++;
              ...
              {
               if($GameAvailable=='ON'){
               $EngagementColumnsArray=array('Date','Stream','Posts','NamedPosts','EventPosts','SurveyPosts','Groups','Hashtags','News','Games');
            }else{
                 $EngagementColumnsArray=array('Date','Stream','Posts','NamedPosts','EventPosts','SurveyPosts','Groups','Hashtags','News','Jobs');
            }
               if ($GameAvailable == 'ON' && $NewsAvailable == 'ON') {
                $EngagementColumnsArray = array('Date','Stream','Posts','NamedPosts','EventPosts','SurveyPosts','Groups','Hashtags','News','Games','Jobs');
            } else if ($GameAvailable == 'ON' && $NewsAvailable == 'OFF') {
                $EngagementColumnsArray = array('Date','Stream','Posts','NamedPosts','EventPosts','SurveyPosts','Groups','Hashtags','Games','Jobs');
            } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'ON') {
                $EngagementColumnsArray = array('Date','Stream','Posts','NamedPosts','EventPosts','SurveyPosts','Groups','Hashtags','News','Jobs');
            } else if ($GameAvailable == 'OFF' && $NewsAvailable == 'OFF') {
                $EngagementColumnsArray = array('Date','Stream','Posts','NamedPosts','EventPosts','SurveyPosts','Groups','Hashtags','Jobs');
            }
...
//                )
 ...
 public function actionTrafficGenerateXLS(){
        ...
        Yii::app()->end();
         ...
         try {
             error_log("in analytics controllerrrrrrrrUsabilityrrrrrrrrrrrrrr");
                if (isset($_POST['startDate']) && $_POST['startDate'] != 'undefined') {
                   $startDate = DateTime::createFromFormat(Yii::app()->params['PHPDateFormat'], $_POST['startDate'])->format("Y-m-d");
               } else {
                   $startDate = date('Y-m-d', strtotime("-7 days"));
               ...
               $SegmentId=$this->tinyObject['SegmentId'];
                ...
                if ($_POST['UsabilityType'] == 'Device') {
                 ...
                 error_log($locationGraphArray['lattitude']."_**__________latitude_________".print_r($locationGraphArray,true));
                 
                      // $heighestValue = CommonUtility::get_highest($locationGraphArray);
                        $result = array("latitude" => $locationGraphArray['lattitude'],"longitude"=>$locationGraphArray['longitude']);
...
$f= preg_replace("/<\/?div[^>]*\>/i", "", Yii::app()->params['COPYRIGHTS']);
     ...
     public function actionGamePdf(){           
       ...
       public function actionEngagementPdf(){
        try{
           $date = $_REQUEST['date'];
           $Titles=array();
           $networkId= $_REQUEST['NetworkId'];
           $Titles=array('Stream','NormalPosts','SurveyPosts','EventPosts','GroupNormalPosts','GroupSurveyPosts','GroupEventPosts','CurbsidePosts','Groups','News','Game','Hashtag','Careers');
           $this->renderPartial('html2pdf',array('FileName'=>'EngagementAnalytics.pdf','GraphType'=>'Engagementhtmlpage','date'=>$date,'configParams'=>Yii::app()->params,"graphs"=>$Titles));
//            
        } catch (Exception $ex) {
