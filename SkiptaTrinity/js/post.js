...
function getsnipetHandler(data) {
    if (data.status == 'success') {
        var divId="snippet_main";
          if($("#editableC").is(':visible')){        
        divId="snippet_mainC";
        }else{
        divId="snippet_main";          
        }
...
function closeSnippetDiv() {
    ...
    $('form #snippet_main,#Groupsnippet_main,#snippet_mainC').hide();
    $("form #snippet_main,#Groupsnippet_main,#snippet_mainC").html("");
...
function initializationForArtifacts() {
    ...
    $("ul[id*=postType] li").click(function() {
        ...
        if (postType == 'Post As Anonymous') {
         ...
         if (postType == "Curbside") {
              $('#surveyeventtitledescription').hide();
              $('#survey_header').hide();
              $('#event_header').hide();
              $('#mainStreamPostWidget').hide();
              $('#curbsidePostCreationdiv').show();
              $('#numero2').show();
              curbsideOnloadEvents();              
              initializationForHashtagsAtMentions('div#editableC');
...
function ClearPostForm() {
    ...
    globalspace['weburls']="";
     $('#surveyeventtitledescription').hide();
     $('#mainStreamPostWidget').show();
              $('#curbsidePostCreationdiv').hide();
             
    $("#curbsidePost-form")[0].reset();
    $('#curbsidedropdown').find('span').text("Please Select Category");
    $('.iisfeatured').val(0);
    $('.isfeatured').removeClass('featureditemenable').addClass('featureditemdisable');
    $('.isfeatured').attr('data-original-title','Mark as Featured');
    ...
    $('#editableC').addClass('placeholder');
    $("#button_block").hide();
...
//                    groupFollowersCount++;
    ...
    $("#" + divId + " img.comments,#" + divId + " img.commented").live("click",
            function() { 
                ...
                if(cmntcnt >0 ){ //don't load for curbside... -- changes in the comment enhancements...
...
//                }
   ...
   $("#" + divId + " .postdetail,#" + divId + " .postdetail_news,#" + divId + " .careerpostdetail").live("click touchstart",
        function() { 

       if($("#contentDiv").hasClass("streamsectionarearightpanel")==true){
        sessionStorage.rightPanelExist = "true";
    }else{
        sessionStorage.rightPanelExist = "false";
    }
       
           var categoryType = $(this).attr('data-categoryType'); 
            if (categoryType != 12 && categoryType != 15){
            if (Number(categoryType) == 9) {
                if(divId=="streamMainDiv"){
                 ...
                 var mode = $('#gameBtn').attr('data-mode');
                   window.location='/games/'+scheduleId+'/detail'; 
                }else{
            ...
            trackEngagementAction("PostDetailOpen",postId,categoryType,postType,'',sourcetype);
            ...
            var id = $(this).data('id'); 
            Global_ScrollHeight = $(document).scrollTop();
            renderPostDetailForCareer(id,'stream');
        }
    ...
    $('.NDESC').live('click',function(){
        ...
        if(parentClass.indexOf("postdetail") == -1){
            var postId = $(this).attr('data-postid');
            var categoryType = $(this).attr('data-categoryType');
            var postType = $(this).attr('data-postType');
            var newsId = $(this).data('id');
    ...
    if(isFeatureType==1){
       ...
       setTimeout(function() { // waiting for 1 sec ... because it is very fast, so that's why we have to forcely wait for a sec.
           if(typeof socketPost !== "undefined"){
                isThereReady = true;                
                socketPost.emit('getLatestPostsRequest', loginUserId, userTypeId, postAsNetwork,sessionStorage.old_key,Project);
            }
 ...
 * @returns {undefined}
 */
function renderPostDetailPage(showDivId, hideDivId, postId, categoryType, postType,recentActivity) { //alert('renderPostDetailPage');
    changeURLForDetail(categoryType,postId);
    if(detectDevices()){
        $("#rightpanel").hide();
    } 
...
function renderPostDetailPageHandler(html,showDivId, hideDivId,categoryType,postId){ 
 ...
 $('.joyride-tip-guide').hide();   
    // $("#contentDiv").removeClass("streamsectionarearightpanel");
            ...
            else if($("#groupPostDetailedDiv").length > 0){ 
...
function trackEngagementAction(action,dataId,categoryType,postType,id,sourceType){    
   ...
   if(sourceType==null || sourceType=="undefined"){
    ...
    var queryString ={"page":gPage,"action":action,"dataId":dataId,"categoryType":categoryType,"postType":postType,"id":id,"sourceType":sourceType};
    //alert(queryString.toSource());
    ajaxRequest("/post/trackEngagementAction",queryString,function(data,action){        
    });
}
...
function renderGameDetailedPageHandler(data,showDivId, hiddenDivId,postId){
    try{ 
     if(data.status=='success'){        
        window.location.href="/games/"+data.currentScheduleId+"/detail";   
