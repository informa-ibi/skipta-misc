...
function CurbsidePostsend(){
    var editorObject = $("#editableC.inputor");
    if(validateAtMentions(editorObject)){
        if($("#editableC").text().length>0){
            if($.trim($('#editableC').text()).length>0){
...
function curbsideOnloadEvents(){
    if($("#editor-toolbar-editableC").is(':visible')==false){
    $('#editableC').freshereditor({editable_selector: "#editableC", excludes: ['removeFormat', 'insertheading4']});
    $("#editableC").freshereditor("edit", true); 
