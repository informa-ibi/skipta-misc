...
function ajaxRequest(url, queryString,callback,dataType,beforeSendCallback) { 
    ...
    var allow=false;
    var allowURLArray = ["site","common","marketresearchview","profile","invite","posts","curbsideconsult","newsdetail","groups","subgroups","games","careers"];
   // if(pathname.toLowerCase().indexOf("site") >= 0 || pathname.toLowerCase().indexOf("common") >= 0 || pathname.toLowerCase().indexOf("marketresearchview") >= 0 ||pathname.toLowerCase().indexOf("profile") >= 0  ||pathname.toLowerCase().indexOf("invite") >= 0 || pathname.toLowerCase().indexOf("posts") >= 0 || pathname.toLowerCase().indexOf("curbsideconsult") >= 0 ||  pathname.toLowerCase().indexOf("newsdetail") >= 0 ||  pathname.toLowerCase().indexOf("subgroups") >= 0 ||  pathname.toLowerCase().indexOf("groups") >= 0){
   var pathNameArray = pathname.split("/");
    if(jQuery.inArray( pathNameArray[1], allowURLArray )>=0){
        allow=true;
    ...
    checkSession();
        ...
        if(isUserSessionValid=="yes" || allow==true)
    {
...
function trackUserSession(activityType){
    ...
    if(activityType == "start"){
        ...
        time = utcTime;
      // time = "Thursday, 25 June 2015 05:08:11 GMT-0400";
        sessionId = sessionStorage.userSessionId;
    }
      var obj = {"sessionId":sessionId,"UserId":loginUserId,"date":time,"activityType":activityType};
    // alert(searchSocket+"---"+obj.toSource());
     sessionStorage.clear();
     localStorage.clear();
     searchSocket.emit('trackUserSession', JSON.stringify(obj));
}
...
function scrollPleaseWait(spinnerId, divId){
    ...
    if($(".surveySavingRes").length>0 && divId != "previous"){
...
function getAllNotificationByUserId(type){    
    if(type == undefined){
        ...
        socketNotifications.emit("getAllNotificationByUserId",loginUserId,startLimit,Project);
    }
   ...
   function loadTopAds(position,page){
    ...
    if (sourceType == "Upload") {
        ...
        if(htm.indexOf("<iframe")==0){
          ...
          $('#StreamBundleAds_' + position).html('<center><iframe width="300" height="250" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" onload="iframeOnLoadHandler(\''+position+'\');" scrolling="no" bordercolor="#000000" src="/advertisements/renderscriptad?id='+ads[id].id+'"></iframe></center>');  
        }
...
function getJsonObjectForNode(){    
    ObjectA = {PF1:pF1,PF2:pF2,PF3:pF3,PF4:pF4,PF5:pF5,sCountTime:socialActionIntervalTime,storiesTime:postIntervalTime,uniquekey:sessionStorage.old_key,pageName:gPage,notTime:notificationTime,utcTime:utcTime,sessionId:localStorage.userSessionId,project:Project};
...
function generateJSONObj(obj){
    ...
    if($(obj).attr('data-posttype')){
        ...
        if($(obj).attr('data-sourcetype')){
            resObj.sourceType = $(obj).attr('data-sourcetype');
        }else{
            resObj.sourceType = 0;
        }
...
function highlightNewUsers(currentIndex,position,fromClick)
{
 ...
 else if( $('#streamMainDiv  div.social_bar:first').length > 0)
 {
...
function highlightNewPosts(currentIndex,position,fromClick)
{
   ...
   adjustJoyRidePosition( $("div[data-index='" + currentIndex + "' ]"),position,$('#streamMainDiv  div.social_bar:first '));;
   if(fromClick=="click")
  {
     if( !$('#streamMainDiv  div.social_bar:first img:first').hasClass("joyrideHighlight"))
     {
        //adjustDivPosition( $('#streamMainDiv > :nth-child(2)'),currentIndex)
        $('#streamMainDiv div.social_bar:first img:first').addClass("joyrideHighlight");
  ...
  var targetObjTop= $('#streamMainDiv  div.social_bar:first ').offset().top;
            var targetObjHeight= $('#streamMainDiv  div.social_bar:first ').closest("div.post").height();
  ...
  if(fromClick=="click")
  {
    if( !$('#jobsListIndex li.jobsLis').hasClass("joyrideHighlight"))
    {
        if($('#jobsListIndex li.jobsLis').length>0){
            ...
            var postionLeft=$('#jobsListIndex li.jobsLis:first').offset().left;
                ...
                if(postionLeft>windowWidth/2)
                {
                    ...
                    $('#jobsListIndex').find('.jobsLis:first').next().addClass("joyrideHighlight") 
                     ...
                     $('#jobsListIndex').find('.jobsLis:first').addClass("joyrideHighlight");
...
function highlightNews(currentIndex,position,fromClick)
{
  ...
  if(fromClick=="click")
  {
    if( ! $("#ProfileInteractionDivContent ul li.newsLis").hasClass("joyrideHighlight"))
    {
          ...
          //var joyrideTop= $("div[data-index='" + currentIndex + "' ]").position().top;
   //adjustDivPosition( $("#ProfileInteractionDivContent ul li:first"),currentIndex)
...
//              $(".advancedtourguide").removeClass("arrow_box_bottom");
       ...
       //alert(toolTipObj+","+position+","+targetObj);
        if( toolTipObj!=null &&  toolTipObj!=undefined && toolTipObj.position()!=null && toolTipObj.position()!='null')
        {
            ...
            /// alert("in bottom"+targetObjLeft +","+tooltipTop);
...
function minimizeJoyride(currentIndex)
{
   ...
   if($.trim($(".joyride-tip-guide").html())!="")
   {
    $(".joyrideHighlight").removeClass("joyrideHighlight");
    $(".joyride-tip-guide").hide();
//    if($("#minChatWidgetDiv").css("display")=="block"){
//        var chatWidth = $("#minChatWidgetDiv div.closedchat").width();
//        $("#minTourGuideDiv").css("left",0);
//        var tourGuideRight = Number(chatWidth)+20;
//        $("#minTourGuideDiv div.closedJoyride").css("margin-right",tourGuideRight);
//    }else{
//        $("#minTourGuideDiv").css("left",0);
//        $("#minTourGuideDiv").css("right",0);
//    }
...
$(function(){
$("#editableC").bind('paste', function (event) {
 var $this = $(this); //save reference to element for use laster
 setTimeout(function(){ //break the callstack to let the event finish
   //  var strippedText = strip_tags($this.html(),'<p><pre><span><i><b><li></li><ul></ul>');
   var posttype=$this.attr('data-type');
...
function autoRenderNewStory(){
     if(userInactive == true){
         autoActionPerfomed = true;
               $("#notificationsdiv").trigger("click");
               setTimeout(function(){
                   autoActionPerfomed = false;
               },6000)
  }
}
function getURLString(categoryId,postId){
...
return categoryType+"/"+postId;
}
function getPagetypeFoAdRendering(categoryId){
    var pageType;
    switch (categoryId) {
    ...
    case "1":
        ...
        break;
    case "2":
        pageType = "Curbside Detail";
        break;
    case "3":
         pageType = "Group Post Detail";
        break;
...
function changeURLForDetail(categoryType,postId){
   
     var changedUrl = getURLString(categoryType,postId); 
    sessionStorage.prevURI = window.location.pathname.substr(1);
     window.history.pushState("object or string", "Title",changedUrl);
}
function handlingbacknavigation(){    
    if(window.history){
        $(window).on('popstate', function() {    
          ...
          if (hashName !== '') {
            ...
            if (hash === '') {
               $("#breadcrumSoureEle").trigger("click");
            }
          }
          });
}
