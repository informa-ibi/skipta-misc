...
try{
var options = {
 key: fs.readFileSync('/etc/nginx/certs/SkiptaTrinity/SkiptaTrinity.key'),
   //  cert: fs.readFileSync('/etc/nginx/certs/SkiptaTrinity/SkiptaTrinity.crt'),
   cert: fs.readFileSync('/etc/nginx/certs/SkiptaTrinity/SkiptaTrinity.pem'),
...
socket2.on('connect', function(){
    ...
    projectSearch = function(search, projectOffset, projectPageLength, userSearch, groupsSearch,subGroupsSearch,hastagsSearch, postSearch,loginUserId,curbsideCategory,gameSearch,socketId,project){
         socket2.emit('projectSearch', search, projectOffset, projectPageLength, userSearch, groupsSearch,subGroupsSearch,hastagsSearch, postSearch,loginUserId,curbsideCategory,gameSearch,socketId,project) ;
    ...
    trackSearchAchievements = function(obj,socketId){
     ...
     trackUserSession = function(obj,socketId){
         socket2.emit('trackUserSession', obj,socketId) ;
 ...
 getUsersForSearch = function(searchdata,socketId){
    ...
    saveBrowseDetails = function(sessionObject,project){
    socket2.emit('saveBrowseDetails', sessionObject,'',project) ;
...
socket2.on('disconnect', function () {
 ...
 socket2.on('trackUserSessionResponse', function(data,socketId) {

 trackUserSessionResponse(data,socketId);
 });
...
socket3.on('connect', function(){
    ...
    getLatestPostsRequest = function(userId,userTypeId,postAsNetwork,clientId,project){
       
         socket3.emit('getLatestPostsRequest', userId,userTypeId,postAsNetwork,clientId,timezoneName,project);
    ...
    clientRequest4GroupPost = function(postids,socketId,jsonObject){
     ...
     getLatestGroupPostRequest = function(userId,groupId, userTypeId,type,postAsNetwork,socketId,project){
         socket3.emit('getLatestGroupPostRequest', userId,groupId, userTypeId,type,postAsNetwork,socketId,timezoneName,project) ;

    }
     getUpdatedStreamPostRequest = function(userId,streamId,userTypeId,pageType,socketId,project){
         socket3.emit('getUpdatedStreamPostRequest', userId,streamId,userTypeId,pageType,socketId,timezoneName,project) ;
...
socket5.on('connect', function(){
    ...
    getAllNotificationByUserId = function(userId,page,socketId,project){
         socket5.emit('getAllNotificationByUserId', userId,page,socketId,project) ;
...
socket4.on('connect', function(){
    ...
    getUserFriends = function(userId, searchText,startLimit,pageLength,callback,socketId,project){
         var userIdsArray = Object.keys(mjmChatUserSockets);
        socket4.emit('getUserFriends', userId, searchText,startLimit,pageLength,callback,userIdsArray,socketId,project) ; 
    }
    searchUsers = function(userId, searchText,startLimit,pageLength,callback,socketId,project){
         var userIdsArray = Object.keys(mjmChatUserSockets);
        socket4.emit('searchUsers', userId, searchText,startLimit,pageLength,callback,userIdsArray,socketId,project) ; 
    }
    ...
    getOfflineMessages = function(loginUserId, callback,socketId,project){
         socket4.emit('getOfflineMessages',loginUserId, callback,socketId,project) ;
    }
...
{    
   ...
   client.on('projectSearch', function(searchText,offset,pageLength,userSearch,groupsSearch,subGroupsSearch,hastagsSearch,postSearch,loginUserId,curbsideCategory,gameSearch,project)
    { 
             projectSearch(searchText, offset, pageLength, userSearch, groupsSearch,subGroupsSearch,hastagsSearch, postSearch,loginUserId,curbsideCategory,gameSearch,client.id,project);
    ...
    { 
     ...
     client.on('trackUserSession', function(obj)
    { 
             trackUserSession(obj,client.id);
...
client.on('getUsersForSearch', function(searchdata)
    { 
     ...
     client.on('saveBrowseDetails', function(sessionObject,project)
    { 
     // console.log("saveBrowseDetails******************"); 
       
       saveBrowseDetails(sessionObject,project);
...
//           client.emit("projectSearchResponse",data);
    ...
    trackUserSessionResponse = function(data,socketId){
        var socketObj;
         var clients = io.sockets.sockets;
            for (var i = 0; i < clients.length; i++) {
              if(clients[i].id == socketId){
            ...
            try{ 
                 console.log("-----track user session response----"+data);
                 var data1 = eval("("+data+")");
                  console.log("-----track user session response----"+data1.activityType);
                  if(data1.activityType == "Logout"){
                     var clients = mjmChatUserSockets[data1.userId];
       
        for (var i = 0; i < clients.length; i++) {
             
              if(clients[i].id != client.id){
                  console.log("refreshp age----------------");
                clients[i].emit("refreshPage");
              }
            }  
                  }
  		if(data!="" && data!=0 && data!=null){
                socketObj.emit("trackUserSessionResponse",data);
                 
                
            }
            }catch(er){
                //logger.trace("Exception Occurred=== in mobileSearchResponse emit====="+er);
                logger.error("Exception Occurred=== in trackUserSessionResponse emit====="+er);
            }
...
//           client.emit("projectSearchResponse",data);
    ...
    { 
        ...
        userActiveSessionTime[job.sessionId] = job.utcTime;
...
//           console.log("getNewPostsResponse=====Data=!!!!!!!!!!!!!!!!!!!========"+data);
     ...
     client.on('getLatestPostsRequest', function(userId,userTypeId,postAsNetwork,hostname,project)
    { 
       ...
       getLatestPostsRequest(userId,userTypeId,postAsNetwork,client.id,project);
    ...
    { 
      ...
      client.on('getLatestGroupPostRequest', function(userId,groupId, userTypeId,type,postAsNetwork,remoteAddress,project)
    { 
       //console.log("getLatestGroupPostRequest---------------groupId-----"+groupId); 
       clearInterval(clientPostInterval[remoteAddress]);
       getLatestGroupPostRequest(userId,groupId, userTypeId,type,postAsNetwork,client.id,project);
     ...
     getLatestGroupPostResponse = function(data,socketId){          
      ...
      client.on('getUpdatedStreamPostRequest', function(userId,streamId,userTypeId,pageType,project)
    { 
       //console.log("getUpdatedStreamPostRequest--------------------"); 
       getUpdatedStreamPostRequest(userId,streamId,userTypeId,pageType,client.id,project);
...
//        getUnreadNotifications(userId,client.id);
            ...
            if (jsonObject != "" && jsonObject != "undefined" && (requestType == "sSetInterval" || requestType == "clearAndSetInterval")) {
       var job = JSON.parse(jsonObject);
        var gPage = job.uniquekey;
              console.log('getUnreadNotifications---------------in-');  
             if(requestType == "clearAndSetInterval"){
                     clearInterval(clientNotificationInterval[gPage]);
                      getUnreadNotifications(userId,client.id,jsonObject);
                }
                userActiveSessionTime[job.sessionId] = job.utcTime;
                console.log(job.sessionId + "---getUnreadNotifications--leng----" + inactiveSessions + "-----" + inactiveSessions.length);
                if (inactiveSessions.indexOf(job.sessionId) >= 0) {
                    removeFromArray(inactiveSessions, job.sessionId)
                    console.log("active session ehre noew----" + inactiveSessions);
                    socket2.emit('checkUserSession', job.sessionId, "");
                }
        var intervalTime = parseInt(job.notTime);
console.log('getUnreadNotifications---------------in---'+job.PF4);
        if (job.PF4 == 1) {
            connections++;
                    if (connections <= maxconnections) {
                clearInterval(clientNotificationInterval[gPage]);
        ...
        getUnreadNotificationsRes = function(data, socketId,jsonObject) {
            ...
            try{
                ...
                var utctime = userActiveSessionTime[job.sessionId];
                 if(inactiveSessions.indexOf(job.sessionId) == -1){
                      inactiveSessions.push(job.sessionId);
                 }
                 console.log("exception------------"+job.sessionId+"---"+utctime);
                var obj = {"sessionId":job.sessionId,"UserId":"","date":utctime,"activityType":"Inactivity"};
                 socket2.emit('trackUserSession', JSON.stringify(obj),"") ;
                if(job != "")
                    killAllIntervals(job.uniquekey);
     ...
     client.on('getAllNotificationByUserId', function(userId,page,project)
    { 
       //console.log("getAllNotificationByUserId--------------------"); 
       getAllNotificationByUserId(userId,page,client.id,project);
...
// 
//    }
     client.on('getUserFriends', function(userId, searchText,startLimit,pageLength,callback,project)
    { 
       //console.log("getUserFriends--------------------"); 
       getUserFriends(userId, searchText,startLimit,pageLength,callback,client.id,project);
...
//               }
    ...
    // delete mjmChatUserSockets[client.userId];
      ...
      client.on('getOfflineMessages', function(loginUserId, callback,project)
    { 
       
       getOfflineMessages(loginUserId, callback,client.id,project);
 ...
 //console.log("------------------"+clients[i].id);
  if(clients[i].id == socketId){
   ...
   client.on('getPostIdsFromClientResponse',function(postIds,jsonObject,socketId,gPage){
           ...
           var job = JSON.parse(jsonObject);
         console.log("11111111111111111111111111111111***=###################=getPostIdsFromClientResponse==in the interval=!!!!!!!!!========"+job.sessionId+"--"+job.utcTime)
        userActiveSessionTime[job.sessionId] = job.utcTime;
