...
io.sockets.on('connection', function(socket) {  
    socket.on('getUnreadNotifications', function(userId,socketId,jsonObject) {
            try {
                  ...
                  logger.error("____***_______"+obj.project);
         if(obj.project=="Trinity"){
         child = spawn(dir+"/yiic", ['ncom', 'getUnreadNotifications', '--userId=' + userId]);     
         }else{
             child = spawn(dir+"/yiicVD", ['ncom', 'getUnreadNotifications', '--userId=' + userId]);
         }
        ...
        child.stderr.on('data', function(data) {
            ...
            logger.error('stderr: ' + data);
                ...
                logger.error("errror occured in getUnreadNotifications***:::"+err);
            }
       
    });
        socket.on('getAllNotificationByUserId', function(userId,page,socketId,project) {
            if(project=="Trinity"){
              child = spawn(dir+"/yiic", ['ncom', 'getAllNotificationByUserId', '--userId=' + userId,'--startLimit='+page]);        
            }else{
              child = spawn(dir+"/yiicVD", ['ncom', 'getAllNotificationByUserId', '--userId=' + userId,'--startLimit='+page]);          
            }
  ...
  socket.on('getBadgesUnlocked', function(data,jsonObject,socketId) {
         ...
         var obj = JSON.parse(jsonObject);
           if(obj.project=="Trinity"){
            ...
            child = spawn(dir+"/yiicVD", ['ncom', 'getBadgesUnlocked', '--userId=' + data.userId, '--isMobile=' + data.isMobile]);       
           }
