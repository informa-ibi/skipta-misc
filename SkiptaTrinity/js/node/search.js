...
{  
    ...
    client.on('uncaughtException', function(err) {
     ...
     client.on('projectSearch', function(searchText,offset,pageLength,userSearch,groupsSearch,subGroupsSearch,hastagsSearch,postSearch,loginUserId,curbsideCategory,gameSearch,socketId,project)
    {  
        if(project=="Trinity"){
          ...
          child = spawn(dir+"/yiicVD", ['ncom', 'projectSearch', '--searchText=' + searchText,'--offset=' + offset,'--pageLength=' + pageLength,'--userSearch=' + userSearch,'--groupsSearch=' + groupsSearch,'--subGroupsSearch=' + subGroupsSearch,'--hastagsSearch=' + hastagsSearch,'--postSearch=' + postSearch,'--loginUserId=' + loginUserId, '--curbsideCategory='+curbsideCategory,'--gameSearch='+gameSearch]);  
        }
...
//           logger.trace('stderr: ' + data);
   ...
   client.on('saveBrowseDetails', function(sessionObject,address,project) {
    ...
    if(project=="Trinity"){
      ...
      child = spawn(dir+"/yiicVD", ['track', 'saveBrowseDetails', '--sessionObj=' + sessionObject,'--clientIP=' + address]);       
    }
...
//        logger.trace('stderr: ' + data);
    ...
    {  
     ...
     client.on('trackUserSession', function(obj,socketId)
    {  
        console.log("trackUserSession------");
       child = spawn(dir+"/yiic", ['track', 'trackUserSession','--obj=' + obj]);
       child.stdout.setEncoding('utf-8');
       child.stdout.on('data', function(data) {
           console.log('trackUserSessionResponse-----');
           if(socketId != ""){
               client.emit('trackUserSessionResponse', data,socketId);  
           }
       ...
       client.on('checkUserSession', function(sessionId)
    {  
        console.log("checkUserSession------");
       child = spawn(dir+"/yiic", ['track', 'checkUserSession','--sessionId=' + sessionId]);
       child.stdout.setEncoding('utf-8');
       child.stdout.on('data', function(data) {
           console.log('checkUserSessionSessionResponse-----');
