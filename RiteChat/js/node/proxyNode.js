...
try{
var options = {
     key: fs.readFileSync('/etc/nginx/certs/RiteChat/RiteChat.key'),
   //  cert: fs.readFileSync('/etc/nginx/certs/RiteChat/RiteChat.crt'),
   cert: fs.readFileSync('/etc/nginx/certs/RiteChat/RiteChat.pem'),
...
socket2.on('connect', function(){
    ...
    projectSearch = function(search, projectOffset, projectPageLength, userSearch, groupsSearch,subGroupsSearch,hastagsSearch, postSearch,loginUserId,curbsideCategory,socketId){
         socket2.emit('projectSearch', search, projectOffset, projectPageLength, userSearch, groupsSearch,subGroupsSearch,hastagsSearch, postSearch,loginUserId,curbsideCategory,socketId) ;
...
socket2.on('disconnect', function () {
 ...
 console.log('testgetUsersForSearch'); 
...
socket3.on('connect', function(){
    ...
    getNewPostsRequest = function(postdt, userId, userTypeId,postAsNetwork,clientId,jObject){
     ...
     getMobileNewStoriesRequest = function(postdt, userId, userTypeId,postAsNetwork,clientId,jObject){
        console.log("getMobileNewStoriesRequest-------------");
    ...
    clearConnectedClientInterval = function(socketId,hostname){
        console.log("==============clearedConnectedClientInterval==================");
    ...
    getMobileLatestPost = function(postdt, userId, userTypeId,postAsNetwork,socketId){
        console.log("==============getMobileLatestPost=========postdt========="+postdt+"==timezoneName==="+timezoneName);
     ...
     getMobileLatestGroupPost = function(postdt, userId,groupId,userTypeId,postAsNetwork,socketId){
        console.log("==============getMobileLatestGroupPost=========postdt========="+postdt+"==timezoneName==="+timezoneName);
     ...
     getMobileLatestCurbPost = function(postdt, userId, userTypeId,postAsNetwork,socketId){
        console.log("==============getMobileLatestCurbPost=========postdt========="+postdt+"==timezoneName==="+timezoneName);
     ...
     getMobileLatestNews = function(postdt, userId, userTypeId,postAsNetwork,socketId){
        console.log("==============getMobileLatestNews=========postdt========="+postdt+"==timezoneName==="+timezoneName);
...
socket5.on('connect', function(){
 ...
 getAllNotificationByUserIdResponse(data,socketId);
  ...
  socket5.on('getBadgesUnlockedRes', function(data,jsonObject,socketId) {
   //console.log("============********socket5.on*****=getBadgesUnlockedRes===============");
...
io.set('authorization', function (handshakeData, cb) {
    ...
    console.log('aurothirzeion:---------------- '+timezoneName+"---"+userId);
...
{    
     /*Search Start*/
       ...
       if (mjmChatUserSockets[userId] != null || mjmChatUserSockets[userId] != undefined) {
            console.log("-connectionifffffffffffffff0--");
            var array = mjmChatUserSockets[userId];
            array.push(client);
             mjmChatUserSockets[userId] = array;
        }else{
             console.log("-connectionelseseeee0--");
              mjmChatUserSockets[userId] = new Array(client);
        }

         client.broadcast.emit('imOnLine', userId);

   client.on('projectSearch', function(searchText,offset,pageLength,userSearch,groupsSearch,subGroupsSearch,hastagsSearch,postSearch,loginUserId,curbsideCategory)
    { 
             projectSearch(searchText, offset, pageLength, userSearch, groupsSearch,subGroupsSearch,hastagsSearch, postSearch,loginUserId,curbsideCategory,client.id);

    });
 client.on('mobileSearch', function(searchdata)
    { 
       console.log("node https mobileSearch--------------------"+searchdata); 
       mobileSearch(searchdata,client.id);

    });
client.on('getUsersForSearch', function(searchdata)
    { 
       console.log("node https getUsersForSearch--------------------"+searchdata); 
       ...
       // console.log("projectSearchResponse--------------------"+socketId); 
         ...
         var clients = io.sockets.sockets;
            ...
            try{
                ...
                console.log("Exception Occurred=== in emit====="+er);
            }
...
//             console.log("------------------"+clients[i].id);
            ...
            try{
                ...
                console.log("=EXCEPTION ############%%%%%%%%%%%%%%%%=serverResponse===Exception Occurred=== in emit====="+er);
                killAllIntervals(job.uniquekey);
                console.log("==serverResponse===Exception Occurred=== in emit====="+er);
            }
...
//           console.log("getNewPostsResponse=====Data=!!!!!!!!!!!!!!!!!!!========"+data);
        ...
        try{
            if(data != "" && data != 0 && data != 1)
               socketObj.emit("getNewPostsResponse",data);
                ...
                killAllIntervals(job.uniquekey);
                console.log("==getNewPostsResponse===Exception Occurred=== in emit====="+er);
        }
           ...
           // console.log("CurbsidePostsResponse--------------------"); 
            ...
            try{
                if(data != "" && data != 0 && data != 1)
                    ...
                    killAllIntervals(job.uniquekey);
                    console.log("==CurbsidePostsResponse===Exception Occurred=== in emit====="+er);
                }
        ...
        serverResponse4GroupPost = function(data, socketId,jsonObject) {
            ...
            try{
                if(data != "" && data != 0 && data != 1)
                    ...
                    killAllIntervals(job.uniquekey);
                    console.log("==serverResponse4GroupPost===Exception Occurred=== in emit====="+er);
                }
        }
    ...
    { 
     ...
     getLatestGroupPostResponse = function(data,socketId){
          console.log("getLatestGroupPostResponse--------------------"+socketId);
         var socketObj;
            var clients = io.sockets.sockets;
            for (var i = 0; i < clients.length; i++) {
                //console.log("------------------"+clients[i].id);
...
//             console.log("------------------"+clients[i].id);
            ...
            try{
                ...
                killAllIntervals(job.uniquekey);
                console.log("==serverResponse4news===Exception Occurred=== in emit====="+er);
            }
...
//             console.log("------------------"+clients[i].id);
            ...
            try{
                ...
                killAllIntervals(job.uniquekey);
                console.log("==serverResponse4games===Exception Occurred=== in emit====="+er);
            }
     ...
     getTopicsRes = function(data,socketId,jsonObject){
                ...
                if(data != "" && data != 0 && data != 1 && data != 'null')
		{
                ...
                killAllIntervals(job.uniquekey);
              console.log("====Exception occurred in topicresponse====="+er);
       ...
       var job = JSON.parse(jsonObject);
        ...
        getUnreadNotificationsRes = function(data, socketId,jsonObject) {
            ...
            try{
                ...
                if(job != "")
                    killAllIntervals(job.uniquekey);
                console.log("===Exception occurred while sending response to a client===="+err);
            }
            ...
            if(connections <= maxconnections){                
                 if (job.PF5 == 1) {                     
                     ...
                     clientTopicsInterval[gPage] = setInterval(function() {
                         console.log("############IN interval getTopicsRequest#########");
//                         getUnreadNotifications(userId,client.id,jsonObject);
           ...
           // console.log("getBadgesUnlockedRes------final--------------"); 
            ...
            try{
                if(data != "" && data != 0 && data != 1)
                    ...
                    killAllIntervals(job.uniquekey);
                   // console.log("==getBadgesUnlockedRes===Exception Occurred=== in emit====="+er);
                }
    ...
    { 
     //  console.log("getUserId--------------------**"+userId+"---"+mjmChatUserSockets[userId]); 
      ...
      //mjmChatUserSockets[userId] = client;
        ...
        if (mjmChatUserSockets[userId] != null || mjmChatUserSockets[userId] != undefined) {
           console.log(userId+"-*************************ifffffffffffffff0-********************-"+mjmChatUserSockets[userId].length);
            var array = mjmChatUserSockets[userId];
            array.push(client);
             mjmChatUserSockets[userId] = array;
        }else{
       ...
       // console.log("afeter getUserId--------------------**"+ mjmChatUserSockets[userId].toString()); 
        ...
        {
            console.log("mjmChatAddUser--------------------"+client.id+"---"+recieverId+"---"+room); 
            ...
            client.join(room);
            if (clientReciever != null || clientReciever != undefined) {
                 console.log("clientreciever---------------"+clientReciever.length);
                 try{
            ...
            console.log("clientReciever join room");
...
//             }
 ...
 console.log("mjmChatMessage---------- "+client.userId+"----------"+client.id+"-----"+room); 
        ...
        client.on('typingStatus', function(roomName, displayName, messageReceiverId, flag)
        {
            ...
            console.log(client.id+"====typingStatus-------------"+roomName+"--------------"+flag+"----"+displayName+"--"+messageReceiverId);
...
//               }
              console.log("length00000000-------------"+mjmChatUserSockets[userId].length);
 ...
 //console.log("------------------"+clients[i].id);
  if(clients[i].id == socketId){
    ...
    loadMoreChatUsersResponse = function(data,socketId){
         console.log("loadMoreChatUsersResponse--------------------"); 
        var socketObj;
        var clients = io.sockets.sockets;
        for (var i = 0; i < clients.length; i++) {
         //console.log("------------------"+clients[i].id);
  ...
  client.on('getMobileLatestGroupPost', function(postdt, userId,groupId,userTypeId,postAsNetwork)
    { 
        console.log("getMobileLatestGroupPost");
     ...
     getMobileLatestGroupPostResponse = function(data,socketId){
      console.log("getMobileLatestGroupPost--------------------"+socketId); 
            var socketObj;
            var clients = io.sockets.sockets;
            for (var i = 0; i < clients.length; i++) {
                console.log("------------------" + clients[i].id);
 ...
 client.on('getMobileLatestCurbPost', function(postdt, userId, userTypeId,postAsNetwork)
   ...
   client.on('getMobileLatestNews', function(postdt, userId, userTypeId,postAsNetwork)
    ...
    { 
        ...
        var jobj = JSON.parse(jObject);
        var gPage = jobj.uniquekey;
        console.log("@@@@getMobileNewStoriesRequest@@@@----unioquekey---"+jObject);
      //  if(jobj.PF2 == 1){
           if(connections <= maxconnections){
                clearInterval(clientPostInterval[gPage]);
                clientPostInterval[gPage] = setInterval(function(){  
                    console.log(jobj.uniquekey+"---mobile--------------------------page---*"+jobj.pageName);
...
//           console.log("getMobileNewStoriesResponse=====Data=!!!!!!!!!!!!!!!!!!!========"+data);
        ...
        try{
            ...
            console.log("nodata-----------------");    
                ...
                killAllIntervals(job.uniquekey);
                console.log("==getMobileNewStoriesResponse===Exception Occurred=== in emit====="+er);
        }
...
function killAllIntervals(uniquekey){
    console.log("Kill all Intervals ================1111111111111111111111111111111111111111111111111==========="+uniquekey+"===connections==="+connections);
