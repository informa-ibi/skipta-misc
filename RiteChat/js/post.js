...
function getsnipetForComment(event, obj,commentId){
    ...
    if (results && event.keyCode == '32') {
        ...
        ajaxRequest("/post/SnippetpriviewPage", queryString, function(data){getsnipetForCommentHandler(data,commentId);});
    }

}

function getsnipetForCommentHandler(data,commentId) {
...
function ClearPostForm() {
    ...
    $('.disablecomment').attr('data-original-title','Disable Comments');
    $('.iisfeatured').val(0); 
    $('.isdfeatured').removeClass('featureditemenable').addClass('featureditemdisable');
    $('.isdfeatured').attr('data-original-title','Mark as Featured');
    ...
    $("#editable").attr("placeholder", "New Post");
    $('#survey_header').hide();
    $('#eventdiv').hide();
    $('#event_header').hide();
    $("#surveypostdiv").removeClass("surveypostdiv");
...
//                    groupFollowersCount++;
    ...
    $("#" + divId + " .PostManagementActions a.delete").live("click touchstart", function() {
     ...
     $("#" + divId + " .PostManagementActions a.suspend").live("click touchstart", function() {
        ...
        var modelType = 'error_modal';
        var title = 'Delete';
        var content = "Are you sure you want to suspend this game?";
        var closeButtonText = 'No';
        var okButtonText = 'Yes';
        var okCallback = deleteCallback;
        var param = '' + streamId + ',' + postId + ',' + categoryType + ',' + networkId + '';
    ...
    $("#" + divId + " .PostManagementActions a.abuse").live("click touchstart", function() {
        ...
        var modelType = 'error_modal';
        var title = 'Release';
        var content = "Are you sure you want to release this game?";
        var closeButtonText = 'No';
        var okButtonText = 'Yes';
        var okCallback = suspendCallback;
        var param = '' + streamId + ',' + postId + ',' + categoryType + ',' + networkId + ',' + actionType + '';
       ...
       $("#" + divId + " .cancelschedule").live("click", function() {
        ...
        var content = "Are you sure you want to cancel schedule this game?";
        var closeButtonText = 'No';
        var okButtonText = 'Yes';
        ...
        function() { 
           ...
           var categoryType = $(this).attr('data-categoryType'); 
            ...
            if (Number(categoryType) == 9) {
               ...
               var scheduleId = $('#gameBtnWall_'+gameId).attr('gameScheduleId');
                 window.location='/'+gameName+'/'+scheduleId+'/detail/game'; 
                }
               }
    ...
    ajaxRequest("/post/MarkOrUnMarkPostAsFeatured",queryString,function(data){MarkPostAsFeaturedtHandler(data,streamId);});
     trackEngagementAction("PostFeatured",postId,categoryType);
...
function sendNormalPostHandler(data,isFeatureType,PostType) {
    ...
    if (data.status == "success") {
        ...
        if(PostType=='Normal Post'){
            $("#sucmsgForStream").html("Your Post has been created successfully!");
        }else if(PostType=='Event'){
            $("#sucmsgForStream").html("Your Event has been posted successfully!");
        }else if(PostType=='Survey'){
             $("#sucmsgForStream").html("Your Survey has been posted successfully!");
        }else if(PostType=='Anonymous') {
             $("#sucmsgForStream").html("Post has been created successfully!");
        }
...
function saveInvitestByUserIdHandler(data, id) {
    ...
    if (data.status == "success") {
        ...
        if (i == 1) {
            msg = atMentions + " is invited successfully";
        } else if(i==2) {
            msg = atMentions + " are invited successfully";
        } else{
            msg = atMentions + " and "+(i-2)+" are invited successfully";
        }
...
function deleteCallback(param) {
    ...
    var categoryType = paramArray[2];
    var networkId = paramArray[3];
    deletePost(streamId, postId, categoryType, networkId);
}
function abuseCallback(param) {
    ...
    abusePost(streamId, postId, actionType, categoryType, networkId);
}
...
function savePostCommentByUserIdForGroupDetailPageHandler(data, id) {
    ...
    if(data.data!="blocked"){
        ...
        $("#commentsAppend_" + id).prepend($("#groupcommentTmpl_render").render(item));
        $('#comment_new_text').html(data.data.CommentText);
        $('#comment_new_text').removeAttr('id');
        $("#preview_commentTextArea_" + id).hide();
        $("#previewul_commentTextArea_" + id).html("");
...
function viewmoreCommentsHandler(html,commentAppendId,postid,categoryType){
 ...
 //            g_commentPage++;
            ...
            var rendercomments=globalspace['comment_'+commentAppendId]*5;
...
function viewmoreCommentsIndetailedPage(URL, postid, streamid, Collectionname, categoryType) {

   var URL = URL + "?postId=" + postid + "&Collectionname=" + Collectionname + "&Page=" + g_commentPage + "&CategoryType=" + categoryType+ "&streamId=" + streamid+ "&PageType=postdetail" ;
   var data="";   
   scrollPleaseWait("commentSpinLoader_" + streamid);
...
function viewmoreCommentsIndetailedPageHandler(html,streamid){
     if (g_commentPage == 0) {
            ...
            var rendercomments=g_commentPage*5;
            if(rendercomments >= totalcomments){
...
function OpenCommentbuttonArea(postId,categoryType) {
    ...
    $("#commentTextArea_" + postId).animate({"min-height": "30px", "max-height": "200px"}, "fast");
...
function renderNewsDetailedPage(showDivId,hiddenDivId,newsId,postId,categoryType,postType){
    try{
        scrollPleaseWait('commentSpinLoader_'+newsId);
        var URL = "/news/renderPostDetailed?postId=" + postId + "&categoryType=" + categoryType + "&postType=" + postType+"&id="+newsId;
        var data="";
