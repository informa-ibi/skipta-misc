...
function sendGroupPostHandler(data, PostType) {
    ...
    if (data.status == "success") {
        ...
        if (PostType == 'Normal Post') {
            $("#sucmsgForStream").html("Your " + groupType + " Post has been created successfully!");
        } else if (PostType == 'Event') {
            $("#sucmsgForStream").html("Your " + groupType + " Event has been posted successfully!");
        } else if (PostType == 'Survey') {
            $("#sucmsgForStream").html("Your " + groupType + "  Survey has been posted successfully!");
        }
...
function bindClickForArtifact()
{
    $(".f_media_picture.artifacts").live('click', function() {
        ...
        if (postId == undefined && (format == "jpg" || format == "jpeg" || format == "png" || format == "png" || format == "gif")) {
...
function getArtifactDetailForGroupHandler(response, offset, offsetArtifact, width, postId, uri, format) {
    ...
    if (format == "jpg" || format == "jpeg" || format == "png" || format == "png" || format == "gif") {
...
function loadGroupConversations(groupId, type) {
    ...
    if (type == 'SubGroup') {
        getCollectionData('/group/groupStream', 'groupId=' + groupId + '&category=SubGroup&StreamPostDisplayBean', 'groupstreamMainDiv', 'No Posts found.', 'That\'s all folks!');
    } else {
        $('#createSubGroup').hide();
        getCollectionData('/group/groupStream', 'groupId=' + groupId + '&category=Group&StreamPostDisplayBean', 'groupstreamMainDiv', 'No Posts found.', 'That\'s all folks!');
    }
...
function groupPreferencesInitializations(groupId) {
    var extensionspref = '"jpg","jpeg","gif","png","tiff"';
    ...
    $('#DisableWebPreview span.checkbox').bind("click",
     function(){
        ...
        changeDisableWebPreview(groupId,isChecked);
     }); 
