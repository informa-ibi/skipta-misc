<script id="categoriesTmpl" type="text/x-jsrender"> 
 ...
 <span id="Hide" class="collapse in"> more <i class="fa fa-chevron-circle-down"></i></span>
   <span id="Show" class="collapse ">close <i class="fa fa-chevron-circle-up"></i></span>
</a>
                            </div>
                {{/if}}
        </div>
        ...
        <div class="tab-pane " id="profile3">
         <div class="collapse curb_hash_cat_minheight" id="hash_tab-content" >
           ...
           <ul class="outerbadges hashtag">
               ...
               <li {{for _id}}id="curbside_hashtag_list_{{>$id}}" class="curbside_hashtag"{{/for}} class=""><b {{for _id}}onclick="CloseFilterData('curbside_hashtag_list_{{>$id}}','curside_hashtag_{{>$id}}')" id="curside_hashtag_{{>$id}}"  {{/for}} style="display:none;cursor: pointer;">X</b><a style="cursor: pointer;"  {{for _id}} onClick="getHashtagsPosts('{{>$id}}')" {{/for}} > <i>{{>HashTagName}}</i> <span class="badge">  {{if CurbsidePostCount!=null}}{{>CurbsidePostCount}} {{else}} 0 {{/if}}</span> </a>
                ...
                {{else}}
                 ...
                 {{/if}}
                  ...
                  {{/for}}
            <!--  <li class="active"><a href="#"><i>Nutrition Support</i> <span class="badge">42</span></a></li>   -->
            </ul>
     {{else}} 
            <div style="text-align:center;">
           No data found
 ...
 <span id="hash_Hide" class="collapse in"> more <i class="fa fa-chevron-circle-down"></i></span>
   <span id="hash_Show" class="collapse ">close <i class="fa fa-chevron-circle-up"></i></span>
</a>
                            </div>
             {{/if}}
        </div>
