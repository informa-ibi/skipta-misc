...
?>
<li class="woomarkLi">    
    <div class="post item" data-id="<?php  echo $data->_id ?>" data-postid="<?php  echo $data->PostId ?>" data-postType="<?php  echo $data->PostType;?>" data-categoryType="<?php  echo $data->CategoryType;?>" data-networkId="<?php  echo $data->NetworkId; ?>" data-scheduleId="<?php  echo $data->CurrentGameScheduleId; ?>" data-gameName="<?php  echo $data->GameName; ?>">  
        <?php $time = $data->CreatedOn ?>
        <div  style="cursor: pointer" class="stream_title paddingt5lr10 postdetail" data-id="<?php  echo $data->_id ?>"> 
                ...
                <a class="<?php if($data->isGroupAdminMember == 'true' && $data->ActionType=='Post' && $data->SubGroupId==0) { echo 'groupIntroDetails'; } else { echo 'userprofilename';} ?> " data-id="<?php if($data->isGroupAdminMember == 'true' && $data->ActionType=='Post' && $data->SubGroupId==0) { echo $data->MainGroupId; } else { echo $data->UserId; } ?>" style="cursor:pointer">
                    <b>
                        <?php if($data->isGroupAdminMember == 'true' && $data->ActionType=='Post' && $data->SubGroupId==0) {
                           echo $data->MainGroupName; 
                        }else{
                            echo $data->UserDisplayName;
                        } ?>
                    </b>
                </a>
            </b><?php  echo $data->StreamNote." ".$data->PostBy." ".$data->PostTypeString ?><span class="userprofilename"><?php if($data->PostType==2 || $data->PostType==3){ if(isset($data->Title) && $data->Title!=""){ echo "- ".$data->Title; };} ?></span><i> <?php  echo $data->PostOn; ?></i>
        </div>
        ...
        <a  class="pull-left img_single postdetail" data-id="<?php  echo $data->_id ?>" data-postid="<?php echo $data->PostId;?>" data-categoryType="<?php echo $data->CategoryType;?>" data-postType="<?php echo $data->PostType;?>"><div class='<?php echo $videoclassName; ?>'><img src="/images/icons/video_icon.png"></div><img  class="postdetail" src="<?php  echo $data->ArtifactIcon ?>"  ></a>
        <?php  }else{ ?>
            <a  class="pull-left img_more postdetail" data-id="<?php  echo $data->_id ?>" data-postid="<?php echo $data->PostId;?>" data-categoryType="<?php echo $data->CategoryType;?>" data-postType="<?php echo $data->PostType;?>"><div class='<?php echo $videoclassName; ?>'><img src="/images/icons/video_icon.png"></div><img src="<?php  echo $data->ArtifactIcon ?>"></a>
        <?php  }?>
            </div>
            <?php } ?>
            <?php } ?>
        <div class="stream_content">
            <div class="media" data-id="<?php echo $data->_id ?>">
                <div class="media-body postdetail bulletsShow" data-id="<?php  echo $data->_id ?>">
                    ...
                    <div><?php  echo $data->MainGroupName;  ?></div>
...
<!--                            <div class="c_suffix"><input type="radio" class="styled" name="<?php  //echo "survey_".$data->PostId ?>" value="OptionFour"></div> -->
            ...
            <?php if(!isset($data->AddSocialActions) || $data->AddSocialActions==1) {?>     
                <div class="social_bar" data-id="<?php  echo $data->_id ?>" data-postid="<?php  echo $data->PostId ?>" data-postType="<?php  echo $data->PostType;?>" data-categoryType="<?php  echo $data->CategoryType;?>" data-networkId="<?php  echo $data->NetworkId; ?>">	
                    <a class="follow_a"><i ><img src="/images/system/spacer.png" class=" tooltiplink <?php echo $data->IsFollowingPost?'follow':'unfollow' ?>" data-placement="bottom" rel="tooltip"  data-original-title="<?php  echo $data->IsFollowingPost?'Unfollow':'Follow' ?>" ></i> <b id="streamFollowUnFollowCount_<?php  echo $data->_id; ?>"><?php  echo $data->FollowCount ?></b></a>
                    <?php if($data->CategoryType !=3 || ($data->CategoryType ==3 && $data->ConversationVisibility == 1)){ ?> <span><i><img  class=" tooltiplink <?php  echo $data->IsLoved?'likes':'unlikes' ?>"  data-placement="bottom" rel="tooltip"  data-original-title="Love" src="/images/system/spacer.png"></i><b id="streamLoveCount_<?php  echo $data->PostId; ?>"><?php  echo $data->LoveCount?></b></span> <?php  }?>
                        <?php if($data->CategoryType<3 && YII::app()->params['Share']=='ON'){ 
                        if(!$data->TwitterShare || !$data->FbShare){ ?>
                        <span class="sharesection" ><i class="tooltiplink" data-toggle="dropdown" rel="tooltip" data-original-title="Share" data-placement="bottom"><img src="/images/system/spacer.png"  class="share postdetail"  ></i><b id="streamShareCount_<?php  echo $data->_id; ?>"><?php  echo (isset($data->ShareCount) && is_int($data->ShareCount))?$data->ShareCount:0?></b>
                        
                         </span><?php }else{?>
                             <span class="sharesection"><i class="tooltiplink" data-toggle="dropdown" rel="tooltip" data-original-title="Share" data-placement="bottom"><img src="/images/system/spacer.png"  class="sharedisable postdetail"  ></i><b id="streamShareCount_<?php  echo $data->_id; ?>"><?php  echo (isset($data->ShareCount) && is_int($data->ShareCount))?$data->ShareCount:0?></b></span>
                             <?php }} ?>
                    ...
                    <span><i><img src="/images/system/spacer.png" class="tooltiplink <?php echo $data->IsCommented?'commented':'comments'?> postdetail" data-id="<?php  echo $data->_id ?>"  data-placement="bottom" rel="tooltip"  data-original-title="Comment"></i><b id="commentCount_<?php  echo $data->PostId; ?>"><?php  echo $data->CommentCount?></b></span>              
                    <?php  }?>
                </div> 
                     <?php } ?>
            </div>
