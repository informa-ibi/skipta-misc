...
public function saveBrowseDetails($sessionObj,$clientIP) {
        try {
            ...
            error_log("@@@@@@@@@@@@@@@@@sddddddddddddd@@@@@@@@@@@@@@@@2");
            Yii::log("-----inside exception ---saveBrowseDetails------" . $exc->getMessage(), 'error', 'application');
        }
    }
    
    
    public function GetAnalyticsReportsBasedonDateRange($startDate,$endDate,$type){
      try {   
         $finalArray = array();
            $finalArray['Year'] = "";
            $labelsArray = array();
...
//            $endDate = CommonUtility::convert_time_zone(strtotime($endDate), "UTC", $timezone);
    ...
    public function GetDevicePieChartUsabilityBasedOnDateRangeAndType($startDate,$endDate){
      try {
          ...
          $finalArray = array();
            ...
            $c = TrackBrowseDetailsCollection::model()->getCollection();
             $results = $c->aggregate(
                    array('$match' => array("Date" => array('$gte' => $startDate, '$lte' => $endDate))), array('$group' => array(
                    '_id' => array(
                            'Device'=>'$Device',
     ...
     public function GetBrowserUsabilityPieChartBasedOnDateRangeAndType($startDate,$endDate){
      try {
           $finalArray = array();
            $startDate = date('Y-m-d', strtotime($startDate));
            $endDate = date('Y-m-d', strtotime($endDate));
    ...
    public function GetLocationLineGraphReportsBasedonDateRange($startDate,$endDate,$type){
      try {   
          $finalArray = array();
            $finalArray['Year'] = "";
            $labelsArray = array();
...
//            $endDate = CommonUtility::convert_time_zone(strtotime($endDate), "UTC", $timezone);
    ...
    public function GetLocationPieChartUsabilityBasedOnDateRangeAndType($startDate,$endDate){
      try {
           $finalArray = array();
            $startDate = date('Y-m-d', strtotime($startDate));
            $endDate = date('Y-m-d', strtotime($endDate));
    ...
    public function isGroupComebackUser($groupId, $SecurityToken)
    {
        try
        {
            ...
            return $return;
             ...
             Yii::log("------------in exception isGroupComebackUser---------- ".$exc->getMessage(), 'error', 'application');
        }
    }
