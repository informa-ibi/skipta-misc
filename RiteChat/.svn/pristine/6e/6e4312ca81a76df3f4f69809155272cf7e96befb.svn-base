...
class ScheduleGameCollection extends EMongoDocument {
 ...
 public static function model($className = __CLASS__) {
   ...
   public function checkGameScheduleForDates($startDate,$endDate){
       try {
          $returnValue='failure';
      ...
      public function getScheduleGameDetailsObject($type,$value){
               try {
          $returnValue = 'failure';
          $criteria = new EMongoCriteria;
         if($type=='Id'){
    ...
    public function startGame($userId,$gameId,$gameScheduleId){
          try {
             ...
             CommonUtility::prepareStreamObject((int)$userId,"Playing",$gameId,9,'',"",time());
              ...
              error_log("excpetion---------------".$ex->getMessage());
             Yii::log("in saveGame".$ex->getMessage(),'error','application');
          }  
      }
...
//              $modifier->addModifier('UserAnswers.$.Points', 'set', (int)$points);
     ...
     public function submitGame($userId,$gameId,$gameScheduleId, $totalTimeSpent=0){
       ...
       Yii::log("in saveGame".$ex->getMessage(),'error','application',$modifier);
         } 
      }
           ...
           // $criteria->setSelect(array('UserAnswers'=>true));
            ...
            $obj = ScheduleGameCollection::model()->find($criteria); 
             if(is_object($obj)){
              ...
              error_log("excpetion-------------------".$ex->getMessage());
       Yii::log("in findUserGameStatus".$ex->getMessage(),'error','application');
...
//            }else{
   ...
   public function getScheduleGameBetweenDatesByGameId($gameId,$startDate,$endDate){
       $returnValue = 'failure';
       try {
      ...
      public function getPreviousOrFutureGameSchedule($gameId,$startDate,$endDate,$type){
          try {
              $returnValue="failure";
              $criteria = new EMongoCriteria();
            $criteria->addCond('GameId', '==', new MongoID($gameId));      
...
//             );
     ...
     public function getSchedulesForGame($gameId){
             $returnValue = 'failure';
         try {
     ...
     public function updateCurrentScheduleGameByToday($gameId, $date) {
        try {
            ...
            $modifier->addModifier('IsCancelSchedule', 'set', 1);
    ...
    public function UpdateScheduleGameById($Gameobj){
        ...
        try {
            ...
            error_log("---------in shedule game update----". $exc->getTraceAsString());
        }
