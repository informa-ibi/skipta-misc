...
function changeStatusType(userid,value) {
    ...
    var queryString = "userid=" + userid + "&value=" + value;
 ...
 */
  ...
  * */
 function saveEditorial(postId,text) {
    var queryString = {postId:postId,EditorialCoverage:text};
    ajaxRequest("/admin/saveEditorial", queryString, function(data){saveEditorialHandler(queryString)});
}
function saveEditorialHandler(data)
{
     $('.EC'+data.postId).hide();
     $('.EDCRO'+data.postId).fadeIn();
     var editorailCoverage=data.EditorialCoverage;
     if(editorailCoverage.length>240)
     {
     editorailCoverage=editorailCoverage.substr(0,240);
     editorailCoverage=editorailCoverage+'<a data-placement="bottom" rel="tooltip"  data-original-title="show more" class="showmore" data-id="'+data.postId+'">...</a>';
     }
      
     $('.EDCRO'+data.postId).html(editorailCoverage);
     applyLayoutContent();
}
...
function manageCuratedPostHandler(data)
{
    ...
    else if(data_p[0]=='R')
    {
        ...
        $("#E_"+data_p[1]).parent('.cus_strip').prepend(addOperations(data_p[1]));
        $('#'+data).remove();
        $("#E_"+data_p[1]).remove();
        $("#D_"+data_p[1]).remove();
        if(!detectDevices()){
...
function initializeAtMentionsForBadge(inputor, badgeId){
    /*
    ...
    */
    ...
    globalspace['invite_at_mention_'+inputorId]=new Array();    
      var invite_at_mention_config = {
          ...
          var data = {searchkey:query,existingUsers:JSON.stringify(globalspace['invite_at_mention_'+inputorId]),badgeId:badgeId};
        ajaxRequest("/user/getusersForCustomBadge",data,function(data){
...
function assignBadges(badgeId){ 
    var atMentions = '';
    $('#inviteTextBox__currentMentions span.dd-tags-close').each(function(index, element){
        var invitedUserId = $(element).attr('data-user-id');
        atMentions += ","+invitedUserId;
    });
    
    if(atMentions.length>0){
               var queryString = "&badgeId=" + badgeId + "&atMentions=" + atMentions;
            ajaxRequest("/user/saveCustomBadgesToUser", queryString, function(data) {
               assignBadgesHandler(badgeId) 
            });
        
    }else{
        $('#InviteTextBox__em').show();
        $('#InviteTextBox__em').fadeOut(3000);
    }
    
}
function assignBadgesHandler(badgeId){  
    $('#InviteTextBox__sc').show();
    $('#InviteTextBox__sc').fadeOut(3000,function(){
    $("#inviteDiv_").hide();    
    $('#inviteTextBox_').val('');
    $('#inviteTextBox__currentMentions').html('');
    $(".custom_badgesul li").removeClass("active");
        }); 
   
}
function cancelAssignBadges(){
    $("#inviteDiv_").hide();
    $('#inviteTextBox_').val('');
    $('#inviteTextBox__currentMentions').html('');
    $(".custom_badgesul li").removeClass("active");
