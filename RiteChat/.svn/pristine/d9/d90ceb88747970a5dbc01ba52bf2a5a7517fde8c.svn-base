...
//        socketCurbside.emit('clearInterval',sessionStorage.old_key);
    ...
    getCollectionData('/disease/getTrendingDetails', 'StreamPostDisplayBean', 'curbsidePostsDiv', 'No Posts found.','That\'s all folks!');
...
<script type="text/javascript">
  ...
  function CloseFilterData(activeid,divid){
       ...
       $("#curbsidePostsDiv").show();
        ...
        page = 1;
        isDuringAjax=false;
         getCollectionData('/disease/getcurbsideposts', 'StreamPostDisplayBean', 'curbsidePostsDiv', 'No Posts found.','That\'s all folks!');
