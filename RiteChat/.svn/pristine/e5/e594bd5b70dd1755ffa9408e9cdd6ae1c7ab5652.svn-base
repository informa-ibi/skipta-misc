...
class CurbsideCategoriesList extends CActiveRecord {
     ...
     */
    public function getCurbsideCategoriesList($filterValue, $searchText, $startLimit=0, $pageLength=10) {
        try {
     ...
     */
     public function getCurbsideCategoryCount($filterValue, $searchText) {
        try {
            $criteria = new CDbCriteria();
            if (trim($filterValue) == "all") {
                $criteria = $criteria;
