...
class GroupController extends Controller {
    ...
    public function actionIndex() {
        ...
        try {
            ...
            $this->render('index', array('newGroupModel' => $newGroupModel, 'userGroupsFollowing' => CJSON::encode($userGroupsFollowing), 'userGroupsFollowingCount' => CJSON::encode(count($userFollowingGroupsCount)), 'canCreateGroup' => $this->userPrivilegeObject->canCreateGroup));
        } catch (Exception $exc) {
...
//                    $this->render('customGroupDetail', array("IsIFrameMode" => $IsIFrameMode, "preferencesModel" => $newGroupCreationModel, "groupStatistics" => $groupStatistics, 'groupPostModel' => $groupPostModel, 'newGroupModel' => $newGroupModel, "iframeURL" => $groupObject->IFrameURL, "groupId" => $groupId));
            ...
            Yii::log($exc->getMessage() . "in group detail controlloer", 'error', 'application');
        }
    }
        ...
        try {
            ...
            if (isset($_POST['GroupCreationForm'])) {
                ...
                if ($errors != '[]') {
                    ...
                    $newGroupModel->UserId = (isset(Yii::app()->session['PostAsNetwork']) && (Yii::app()->session['PostAsNetwork'] == 1)) ? Yii::app()->session['NetworkAdminUserId'] : $this->tinyObject['UserId'];
                    ...
                    if ($groupId != 'failure' && $groupId != 'groupexists') {
                        if (isset(Yii::app()->session['PostAsNetwork']) && (Yii::app()->session['PostAsNetwork'] == 1) && Yii::app()->params['PostAsNetwork']=='ON') {
                            ServiceFactory::getSkiptaPostServiceInstance()->saveFollowOrUnfollowToGroup($groupId, $this->tinyObject['UserId'], 'Follow');
                        }
                        //$message = Yii::t('translation', 'GroupCreation_Saving_Success');
                        $message = "Toolbox created successfully.";
                        $obj = array('status' => 'success', 'data' => $message);
                    } elseif ($groupId == 'groupexists') {
                        $message = "Toolbox already exists.";
            ...
            Yii::log("----------------in exception ---actionCreateNewGroup-------------------" . $ex->getMessage(), 'error', 'application');
        }
    }
    ...
    public function actiongetMoreFollowingGroups() {
        try {
            ...
            Yii::log("----------------in exception ---actiongetMoreFollowingGroups-------------------" . $ex->getMessage(), 'error', 'application');
        }
    }
    ...
    public function actionGetJoinMoreGroups() {
        try {
            ...
            Yii::log("----------------in exception ---getJoinMoreGroups-------------------" . $ex->getMessage(), 'error', 'application');
        }
    }
    ...
    public function actionUserFollowGroup() {
        try {
            ...
            Yii::log("------------in exception ---actionUserFollowGroup--------------" . $ex->getMessage(), 'error', 'application');
        ...
        try {
            ...
            if (isset($_POST['GroupPostForm'])) {
                ...
                if ($errors != '[]') {
                    ...
                    $groupPostModel->Type = $groupPostModel->Type;
                    $groupPostModel->PostedBy = (isset(Yii::app()->session['PostAsNetwork']) && (Yii::app()->session['PostAsNetwork'] == 1)) ? $this->tinyObject['UserId'] : '';
                    $groupPostModel->UserId = (isset(Yii::app()->session['PostAsNetwork']) && (Yii::app()->session['PostAsNetwork'] == 1)) ? Yii::app()->session['NetworkAdminUserId'] : $this->tinyObject['UserId'];
                    ...
                    if ($errormessage != "") {
                        ...
                        if ($postObj != 'failure') {
                            $message = Yii::t('translation', 'GroupPost_Saving_Success');
    ...
    public function actionGetArtifactDetail() {
        try {
            ...
            $this->renderPartial('artifactsDetail', array('postDetails' => $postDetails));
        } catch (Exception $exc) {
            Yii::log($exc->getMessage(), "error", "application");
        }
    }
    ...
    public function actionGroupStream() {
        ...
        if (isset($_GET['StreamPostDisplayBean_page'])) {
            ...
            if ($category == 'SubGroup') { 
                ...
                $streamData = $this->renderPartial('subgroup_posts_view', array('stream' => $stream, 'pageType' => $category,"disableWebPreview"=>$disableWebPreview), true);
            } else {
                 $userId = $this->tinyObject['UserId'];
                $disableWebPreview = ServiceFactory::getSkiptaPostServiceInstance()->getGroupDisableWebPreviewByGroupId($groupId, "Group");
                $streamData = $this->renderPartial('group_posts_view', array('stream' => $stream, 'pageType' => $category,"disableWebPreview"=>$disableWebPreview), true);
            }
        ...
        try {
            ...
            if (isset($_POST['SubGroupCreationForm'])) {
                ...
                if ($errors != '[]') {
                    ...
                    $subGroupId = ServiceFactory::getSkiptaGroupServiceInstance()->createNewSubGroup($newGroupModel, $UserId);
                    if ($subGroupId != 'failure' && $subGroupId != 'groupexists') {
                        //$message = Yii::t('translation', 'GroupCreation_Saving_Success');
                        $message = "Sub Toolbox created successfully.";
                        $obj = array('status' => 'success', 'data' => $message);
                        if (isset(Yii::app()->session['PostAsNetwork']) && (Yii::app()->session['PostAsNetwork'] == 1)) {
                            $sg_res = ServiceFactory::getSkiptaGroupServiceInstance()->saveFollowOrUnfollowToSubGroup($newGroupModel->GroupId, (int) $this->tinyObject['UserId'], 'Follow', $subGroupId);
                        }
                    } elseif ($subGroupId == 'groupexists') {
                        $message = "Sub Toolbox already exists.";
    ...
    public function actionGetSubGroups() {
        try {
            ...
            $this->renderPartial('subGroups', array('newGroupModel' => $newSubGroupModel, 'groupId' => $groupId, 'groupName' => $groupDetails->GroupName));
        } catch (Exception $exc) {
            Yii::log($exc->getMessage(), "error", "application");
        }
    }
    ...
    public function actionGetJoinMoreSubGroups() {
        try {
            if (isset($_GET['SubGroupCollection_page'])) {
                ...
                $this->renderPartial('subGroups_view', array('stream' => $stream, 'groupName' => $groupDetails->GroupName));
            }
        } catch (Exception $ex) {
            Yii::log("----------------in exception ---getJoinMoreGroups-------------------" . $ex->getMessage(), 'error', 'application');
        }
    ...
    public function actionSubGroupDetail() {
        try {
            ...
            if ($subgroupId != "failure") {
                ...
                $showPostOption = ($this->userPrivilegeObject->canSurvey == 1 || $this->userPrivilegeObject->canEvent == 1) ? 1 : 0;
                $this->render('subgroupDetail', array("groupStatistics" => $groupStatistics, 'newGroupModel' => $newGroupModel, 'groupPostModel' => $groupPostModel, 'showPostOption' => $showPostOption));
            }
        } catch (Exception $exc) {
            Yii::log($exc->getMessage() . "in group detail controlloer", 'error', 'application');
        }
    ...
    public function actiongetGroupPostWidget() {
        try {
            if (isset($_REQUEST['groupId'])) {
                ...
                $showPostOption = ($this->userPrivilegeObject->canSurvey == 1 || $this->userPrivilegeObject->canEvent == 1) ? 1 : 0;
                $this->renderPartial('iframeGroupPost', array('groupPostModel' => $groupPostModel, 'showPostOption' => $showPostOption, 'IsPrivateGroup' => $isGroupPrivate,'groupId'=>$groupId,'DisableWebPreview'=>$groupDetails->DisableWebPreview));
            }
        } catch (Exception $exc) {
            Yii::log($exc->getMessage(), 'error', 'application');
        }
...
//            $returnValue = ServiceFactory::getSkiptaPostServiceInstance()->SaveGroupTopLeadersOfTheDay($groupId);exit;
    ...
    public function actionIsUserFollowAGroup() {
        try {
            ...
            error_log("#########Exception Occurred in actionIsUserFollowAGroup######" . $ex->getMessage());
        }
    }
    ...
    public function actionFollowAllGroups() {
        try {
            ...
            foreach ($groupDetailObj as $key => $group) {
                ...
                $returnValue = ServiceFactory::getSkiptaPostServiceInstance()->saveFollowOrUnfollowToGroup($groupId, $userId, 'Follow');
            }
...
if ($isChecked == 1) {
  ...
  public function actionChangeSubGroupDisableWebPreview() {
      ...
      /**
     * @Author:Ankit
     * Description: Method to Delete group
     * @returntype: json object
     */
    public function actionDeleteGroup() {
        try {
            ...
            $isFollowingGroup = '';
            if (isset($_REQUEST['groupId']) ) {
               $result = ServiceFactory::getSkiptaPostServiceInstance()->saveDeleteToGroup($_REQUEST['groupId']);
                 if ($result == "failure") {
                    throw new Exception('Unable to Delete Toolbox');
                }
               
            }
            $userObj = Yii::app()->session['UserStaticData'];
            Yii::app()->session['UserFollowingGroups'] = ServiceFactory::getSkiptaUserServiceInstance()->groupsUserFollowing($userObj->UserId);
    ...
    public function actionmanagetoolbox() {
        ...
        try {
            ...
            $userGroupsFollowing = ServiceFactory::getSkiptaUserServiceInstance()->groupsUserFollowing($UserId);
            $userFollowingGroupsCount = ServiceFactory::getSkiptaUserServiceInstance()->getUserFollowingGroups($UserId);
            $this->render('managetoolbox', array('newGroupModel' => $newGroupModel, 'userGroupsFollowing' => CJSON::encode($userGroupsFollowing), 'userGroupsFollowingCount' => CJSON::encode(count($userFollowingGroupsCount)), 'canCreateGroup' => $this->userPrivilegeObject->canCreateGroup));
        } catch (Exception $exc) {
            Yii::log($exc->getMessage(), 'error', 'application');
        }
    }
     ...
     * @returntype: groups array
     */
    public function actionGetAllGroupsToManage() {
        try {
            if (isset($_GET['GroupCollection_page'])) {
                $userId = $this->tinyObject['UserId'];
                $pageSize = 4;
                ...
                $this->renderPartial('group_view_all', array('stream' => $stream));
            }
        } catch (Exception $ex) {
            Yii::log("----------------in exception ---getJoinMoreGroups-------------------" . $ex->getMessage(), 'error', 'application');
        }
