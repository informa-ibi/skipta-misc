...
//                        $customMenu["MenuPosition"] = $menu->MenuPosition;
            ...
            echo json_encode(array('status'=>"failure","code"=>$exc->getCode(),'message'=>$exc->getMessage()));
        }
    }
         ...
         $decode=array();
              ...
              //  error_log("decode----".print_r($decode,true));
                if(!is_array($decode) && !count($decode)>0){
                    ...
                    error_log($text."---".$description."--".$title."---".$image);
       ...
       $WebUrls['Weburl'] = trim($text);
            // $SnippetObj->Weburl=trim($text);
                ...
                $webImage="";
                     ...
                     error_log("weburl----".print_r($WebUrls,true));
                     return $WebUrls;
    }
    }
    ...
    public function actionGroupStream() {
        try{
            ...
            if (isset($_GET['StreamPostDisplayBean_page'])) {
                ...
                error_log($groupId."====".$category."====".$UserId);
                ...
                error_log($provider->getTotalItemCount()."======count=====".$provider->getItemCount());
                $stream = -1;
                if ($provider->getTotalItemCount() == 0) {
                    $stream = 0; //No posts
                    ...
                    if ($category == 'SubGroup') {
                        ...
                        $streamRes = (object) (CommonUtility::prepareStreamDataForMobile_V3($UserId, $dataArray, '', 0, '', $timezone,$previousStreamIdArray));
                    }
    ...
    public function actionUserFollowGroup() {
      ...
      public function actionGetGroups() {
            ...
            if (isset($_GET['GroupCollection_page'])) {
               ...
               $descriptionLength = strlen($group->GroupDescription);
                if ($descriptionLength > 240 && $descriptionLength < 500) {
                    $appendData = '<span class="seemore tooltiplink"  data-placement="bottom" rel="tooltip"  data-original-title="See More" onclick="showFullDescription(' . "'" . $group->_id . "'" . ')"> <i class="fa fa-ellipsis-h moreicon moreiconcolor"></i></span>';
                } else {

                    $appendData = ' <span class="postdetail tooltiplink" data-id=' . $data->_id . ' data-placement="bottom" rel="tooltip"  data-original-title="See More" > <i class="fa fa-ellipsis-h moreicon moreiconcolor"></i></span>';
                }
                if ($descriptionLength > 240) {
                    $description = CommonUtility::truncateHtml($group->GroupDescription, 240, '...', true, true, $appendData);
       ...
       public function actionMobileCreateGroupPost() {
         error_log("------------------actionMobileCreateGroupPost----------------------");
