...
class Utils {
     ...
     */
    public static function validateElement(DOMDocument $root) {
        ...
        $signatureElement = self::xpQuery($root, '/saml:Assertion/ds:Signature');
...
//        }
    ...
    public static function xpQuery(DOMNode $node, $query) {
        ...
        if ($xpCache === NULL || !$xpCache->document->isSameNode($doc)) {
            ...
            $xpCache->registerNamespace('samlp', 'urn:oasis:names:tc:SAML:2.0:protocol');
            $xpCache->registerNamespace('saml', 'urn:oasis:names:tc:SAML:2.0:assertion');
    ...
    public static function insertSignature(XMLSecurityKey $key, array $certificates, DOMElement $root, DOMNode $insertBefore = NULL) {
        ...
        $objXMLSecDSig->addReferenceList(
                array($root), $type, array('http://www.w3.org/2000/09/xmldsig#enveloped-signature', XMLSecurityDSig::EXC_C14N), array('id_name' => 'ID', 'overwrite' => FALSE)
        );
    ...
    public static function decryptElement(DOMElement $encryptedData, XMLSecurityKey $inputKey, array $blacklist = array()) {
        ...
        try {
            $res = self::_decryptElement($encryptedData, $inputKey, $blacklist);
            return $res;
        } catch (Exception $e) {
            error_log("DCRE".$e->getMessage());
            /*
     ...
     * @return bool
     */
    public static function validateTimestamps($node) {
        $rootNode = $node;
        $timestampNodes = $rootNode->getElementsByTagName('Conditions');
        $nbAttribute=$naAttribute=$NAD=$NBD=$FNBD=$FNAD='';
        $return=true;
        for ($i = 0; $i < $timestampNodes->length; $i++) {
            ...
            $NBD = date('r',strtotime($nbAttribute->textContent));
            $NAD = date('r',strtotime($naAttribute->textContent));
            $FNBD = date('Y-m-d H:i:s',strtotime($NBD));
            $FNAD = date('Y-m-d H:i:s',strtotime($NAD));
        }
          
           error_log(date('Y-m-d H:i:s')."-----".$nbAttribute->textContent."**ClientTime NOT B4***".$NBD);
           error_log($naAttribute->textContent."**ClientTime NOT After***".$NAD);
           error_log($FNBD);
           error_log($FNAD);
       if ((strtotime($FNBD)>time()) || (strtotime($FNAD)<= time()))
       {
             $return = false;
       }
             return $return;
