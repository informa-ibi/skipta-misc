...
public function saveNewWebSnippet($weburl,$SnippetObj, $createdDate='') {
        try {
           ...
           // $weburl = preg_replace($pattern,'', $weburl);
            ...
            Yii::log("-----inside exception ---SaveNewWebSnippet------".$exc->getMessage(), 'error', 'application');
        }
    }
...
public function CheckWebUrlExist($weburl) {
        try {
           // error_log("in snippet check 111111111".$weburl);
            $returnValue = 'failure';
            $weburl=trim($weburl);
            ...
            Yii::log("______in check weburl exist or not _______".$exc->getMessage(),'error','application');
            return $returnValue;
        }
    }
