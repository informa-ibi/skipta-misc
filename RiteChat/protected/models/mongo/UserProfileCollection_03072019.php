  ...
  /* This method is used for User Authentication which accepts email and password given by the user 
    * it returns string  
    ...
    public function followAction($userId,$followId){
       try{
           ...
           Yii::log("Exception Occurred in FollowAction==".$ex->getMessage(),"error","application");
           return "failure";
       }
       return $result;
   }
   ...
   public function unFollowAction($userId,$followId){
       try{  
           ...
           Yii::log("Exception Occurred in UnFollowAction==".$ex->getMessage(),"error","application");
       }
       return $result;
   }
    /**
    ...
    */
    public function getUserFollowersAndFollowingsById($userId) {
        ...
        try {
            ...
            Yii::log("==getUserFollowersAndFollowingsById in UserProfileCollection==".$exc->getMessage(), 'error', 'application');
        }
        return $returnValue;
    }
    ...
    */
    public function getUserCollectionByUserId($userId,$loggedUserId) {
        try {
            ...
            Yii::log("==getUserCollectionByUserId in UserProfileCollection==".$exc->getMessage(), 'error', 'application');
        }
    ...
    */
    ...
    public function getUserFollowingGroups($UserId)
    {
        ...
        try
        {
            $groupcriteria = new EMongoCriteria;
            $groupcriteria->addCond('IsDeleted', '==', 0);
            $Allgroups = GroupCollection::model()->findAll($groupcriteria);
            $groupname_array = [];
            if(!empty($Allgroups)){
              foreach($Allgroups as $data){
                $groupname_array[]= $data['_id'];
              }
            }
            ...
            $updatedGroupFollowings = [];
            if (isset($groupsFollowing)) {
              if(!empty($groupsFollowing->groupsFollowing) && !empty($groupname_array) ){
                foreach($groupsFollowing->groupsFollowing as $groupId){
                  $groupId = (array)$groupId;
                  if(in_array($groupId['$id'], $groupname_array)  ){
                    $updatedGroupFollowings[] = new MongoId($groupId['$id']);
                  }
                  
                }
                $groupsFollowing->groupsFollowing = $updatedGroupFollowings;
              }
             // print_r($updatedGroupFollowings);exit;
            ...
            return $returnValue;
                ...
                Yii::log("------getUserFollowingGroups------".$ex->getMessage(), 'error', 'application');
         }
    }
   ...
   public function updateUserProfileForGroupActivity($userId,$groupId){
       try {
           ...
           Yii::log("-----in updateUserProfileForGroupActivity---------".$exc->getMessage(), 'error', 'application');
       }
      } 
     /**
     * @author Vamsi Krishna
   ...
   public function getUserGroupActivity($userId)   {
       try {
           ...
           if(isset($userGroupActivity->userVisitedGroupIds)){
    ...
    public function getUserProfileCollection($userId) {
        ...
        try {
           ...
           return $returnValue;
            ...
            Yii::log("-----in updateUserProfileForG", 'error', 'application');
             return $returnValue;
        }
    }
    ...
    */
     ...
     public function getUserFollowingSubGroups($UserId,$groupId)
    {
        ...
        try
        {
            ...
            error_log("--------------exception in user profile collection-----------------------".$ex->getMessage());
                Yii::log("------getUserFollowingGroups------".$ex->getMessage(), 'error', 'application');
         }
    }
 ...
 */ 
    ...
    $returnValue='failure';
        try {
           ...
           return $returnValue;
            ...
            Yii::log("-----in updateUserProfileForG".$exc->getMessage(), 'error', 'application');
             return $returnValue;
        }
        }
    ...
    */
    public function getUserFollowingSliceById($userId,$pageLength,$pageSize) {
        ...
        try {
           ...
           $var=$cursor->getNext();
           $returnValue =isset($var['userFollowing'])?$var['userFollowing']:$returnValue;
          } catch (Exception $exc) {
            Yii::log("====".$exc->getMessage(), 'error', 'application');
        }
        return $returnValue;
    ...
    */
        ...
        try {
           ...
           $var=$cursor->getNext();
           $returnValue =isset($var['userFollowers'])?$var['userFollowers']:$returnValue;
           } catch (Exception $exc) {
            Yii::log("====".$exc->getMessage(), 'error', 'application');
        }
        return $returnValue;
