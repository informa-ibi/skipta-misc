            ...
            <?php echo $form->hiddenField($webLinkForm, 'LinkGroup'); ?>
              <div id="WebLinkSpinner"></div>
               ...
               <?php echo $form->error($webLinkForm, 'LinkGroup'); ?>
                ...
                ?>
            <?php echo CHtml::resetButton('Cancel', array("id" => 'NewWebLinkReset', 'class' => 'btn btn_gray', 'onclick' => 'closeWebLink()')); ?>
...
//    Custom.init();    
    ...
    function getLinkGroupsHandler(data,id){        
        ...
        $('#'+id).append("<option value='0'>Please Choose Link Group</option>");
        $.each(dataArr, function(i){           
            $('#'+id).append($("<option></option>")
            .attr("value",dataArr[i]['id'])
            .text(dataArr[i]['LinkGroupName']));
        });
        $('#'+id).append("<option value='other'>Other</option>");
    }
...
//    );
  ...
  getCollectionData('/weblink/loadWebLinks', 'WLPage=WLWall'+'&WebLinks', 'WebLinkWD', 'No quick links found','No more quick links');
       ...
       scrollPleaseWaitClose("WebLinkSpinner");
        ...
        $("#sucmsgForWL").fadeOut(3000).promise().done(function(){
          $('#addNewWebLink').hide();
              ...
              $('#WebLinkWD').html('');
             page=1;
        getCollectionData('/weblink/loadWebLinks', 'WLPage=WLWall'+'&WebLinks', 'WebLinkWD', 'No weblinks found','No more weblinks');
           //  applyLayout();
          }else{
