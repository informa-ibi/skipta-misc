...
<div  class="groupbanner positionrelative editicondiv" >    
    ...
    <div class="edit_iconbg tooltiplink cursor" data-original-title="Upload  1180 x 200 dimensions" rel="tooltip" data-placement="bottom">
...
<img style="max-width:100%" src="<?php   echo $groupStatistics->GroupBannerImage?>"  id="GroupBannerPreview"/>
 ...
 <div class="row-fluid" id="groupProfileDiv">
    ...
    <ul>
        ...
        <li class="normal">
            <div class="menubox" id="GroupFollowers" <?php  if($groupStatistics->IsFollowing==1 || Yii::app()->session['PostAsNetwork']==1){?> onclick="getUserFollowers('<?php  echo $groupStatistics->GroupId?>','Group') <?php  }?>">
                <div class="menuboxpopup"><span>Followers</span></div>
    ...
    <?php }?>
    <?php if(count($groupStatistics->GroupImagesAndVideosCount)>0 ){?>
    <li id="GroupImages" class="normal" <?php if(count($groupStatistics->GroupImagesAndVideosCount)>0 && ($groupStatistics->IsFollowing==1  || Yii::app()->session['PostAsNetwork']==1)){?> onclick="getGroupImagesAndVideos('<?php   echo $groupStatistics->GroupId?>','Group')" <?php   }?> >
     <div class="menubox" >
         <div class="menuboxpopup"><span>Media</span></div>
     ...
     <?php } ?>
     <?php   if(count($groupStatistics->GroupArtifactsCount)>0 ){?>
       <li id="GroupDocs" class="normal" <?php   if(count($groupStatistics->GroupArtifactsCount)>0 && ($groupStatistics->IsFollowing==1  || Yii::app()->session['PostAsNetwork']==1)){?> onclick="getGetGroupDocs('<?php   echo $groupStatistics->GroupId?>','Group')" <?php   }?> >
     <div class="menubox" >
         <div class="menuboxpopup"><span>Resources</span></div>
...
?>
     ...
     <?php } ?>
     <?php   if($groupStatistics->IsFollowing== 1){?>
     <li id="conversations" class="normal" <?php   if($groupStatistics->IsFollowing==1 || Yii::app()->session['PostAsNetwork']==1 ){?> onclick="loadGroupConversations('<?php echo $groupStatistics->GroupId ?>','Group')" <?php   }?>>
     <div class="menubox">
         <div class="menuboxpopup"><span>Conversations</span></div>
     ...
     <?php if (Yii::app()->params['SubGroups'] == 'ON') { ?>
       <li id="SubGroups" class="normal" <?php   if(count($groupStatistics->SubgroupsCount)!=0 ){?> onclick="getSubGroups('<?php   echo $groupStatistics->GroupId?>')" <?php   }?> >
     <div class="menubox" >
         <div class="menuboxpopup"><span>SubGroups</span></div>
     <div class="groupmenucount">
...
<div id="createSubGroup" style="display: none">
    <div class="row-fluid" >
    <div class="span3"><h2 class="pagetitle">Sub Groups </h2></div>
    <div class="span9 ">
        ...
        <div class="searchgroups padding8top">
            ...
            <input class="btn " id='addSubGroup' name="commit" type="submit" data-toggle="dropdown" value="Add Sub Group" /> 
...
<div id="addNewSubGroup" class="dropdown dropdown-menu actionmorediv actionmoredivtop newgrouppopup newgrouppopupdivtop" >
			...
			<div class="headerpoptitle_white"><?php echo Yii::t('translation', 'Sub_Group_Creation');?></div>
                ...
                <div class="row-fluid  ">
                    <div class="span12">
                       ...
                       <?php echo $form->labelEx($newGroupModel, Yii::t('translation', 'SubGroupName')); ?>
                        <?php echo $form->textField($newGroupModel, 'SubGroupName', array("placeholder" => Yii::t('translation', ''), 'maxlength' => 50, 'class' => 'span12 textfield')); ?> 
                ...
                <div class="groupcreationbuttonstyle alignright">
                        ...
                        ?>
                        <?php echo CHtml::resetButton('Cancel', array("id" => 'NewSubGroupReset', 'class' => 'btn btn_gray')); ?>
...
<div  class="poststreamwidget" id="groupFormDiv" style="display:none">
 ...
 <div class="row-fluid  customrow-fluid" >
   ...
   <div id="editable"  placeholder="New Post" class="placeholder inputor" data-type="group" isWebPreview="<?php echo $groupStatistics->DisableWebPreview?>" contentEditable="true" <?php if($groupStatistics->DisableWebPreview==0){?> onkeyup="getsnipetForGroup(event,this);" <?php }?> onblur="validateDescription(this)"></div>
    <div class="control-group controlerror">  
        ...
        <div class="pull-left whitespace">
            ...
            <ul><li class="dropdown pull-left ">
                   ...
                   <li class="dropdown pull-left" style="white-space:nowrap">
                     <a id="drop2" style="cursor: pointer;" data-toggle="dropdown" class="tooltiplink actionmore " lass="tooltiplink" data-placement="bottom" rel="tooltip" data-original-title="Advanced Options" ><i class="fa fa-gear"><span class="fa fa-caret-down"></span></i></a>
...
<?php  }?>
                   <?php if(Yii::app()->params['DisableComments']=='1'){?>
                    <li>
                        ...
                        <i class="fa fa-comments disablecomments tooltiplink disablecomment" style="position:relative" lass="tooltiplink" data-placement="bottom" rel="tooltip" data-original-title="Disable Comment" ></i> 
                    </li>
                    <?php }?>
   ...
   <?php   echo $form->error($groupPostModel, 'Artifacts'); ?>
    ...
    <div class="pull-right">
         <?php   echo CHtml::Button('Post',array('class' => 'btn','onclick'=>'groupsend('.$groupStatistics->IsPrivate.');')); ?> 
        <?php   echo CHtml::resetButton('Cancel', array("id" => 'grouppostReset','class' => 'btn btn_gray','onclick'=>'ClearGroupPostForm();')); ?>
        </div></div>
...
<?php $this->endWidget(); ?>
    ...
    <div class="sidebar-nav_right" id="groupSideBar"  >
...
//        socketGroup.emit('clearInterval',sessionStorage.old_key);
    ...
    <?php  }?>
     extensions='"jpg","jpeg","gif","mov","mp4","mp3","txt","doc","docx","pptx","pdf","ppt","xls","xlsx","avi","png","tiff","mov","flv"';
     ...
     initializeFileUploader('uploadFile', '/group/upload', '10*1024*1024', extensions,4, 'GroupPostForm' ,'',GroupPreviewImage,appendErrorMessages,"uploadlist_file");
      ...
      getCollectionData('/group/groupStream', 'groupId=<?php   echo $groupPostModel->GroupId ?>&category=Group&StreamPostDisplayBean', 'groupstreamMainDiv', 'No Posts found.','That\'s all folks!',"<?php   echo $groupPostModel->GroupId ?>");
   ...
   <?php } ?>
       function setIframeHeight(iframe) { 
            if (iframe) {
                ...
                alert(iframe.contentDocument.parentWindow)
