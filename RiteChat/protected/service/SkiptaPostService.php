...
////---------------------------Comeback Users---------------------------------------------------------------------------------//
    ...
    /**
     * @author Ankit
     * @usage This is used to get last 6 months login details reports
     * @return string
     */
    public function GetUserLoginData($startDate, $endDate, $type) {
        ...
        try {
            $finalArray = UserLoginDetailsCollection::model()->GetUserLoginData($startDate, $endDate, $type);
    ...
    public function saveUserLoginData($userId) {
        $return = UserLoginDetailsCollection::model()->saveUserLoginData($userId);
//        if ($return == '') {
//            $curatedObject->save();
...
//  
 ...
 /**
    ...
    */
     ...
     * @return type
     */
    public function saveDeleteToGroup($groupId) {
        try {
            $returnValue = 'failure';
            $returnValue = GroupCollection::model()->deleteGroup($groupId, $userId, $actionType);
            return $returnValue;
        } catch (Exception $ex) {
            Yii::log("----------in exception --saveFollowOrUnfollowToGroup-------------" . $ex->getMessage(), 'error', 'application');
            return "failure";
